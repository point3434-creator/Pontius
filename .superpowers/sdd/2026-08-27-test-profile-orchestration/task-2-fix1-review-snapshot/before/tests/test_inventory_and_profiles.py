                ...
                class LooksLikeTestsButIsNotATestCase:
                    def test_helper(self):
                        subprocess.run(
                            ...
                            cwd=".", env={"SAFE": "1"}, timeout=5, check=False,
                        )
                """
            ),
    ...
    def test_working_discovery_binds_every_entry_and_introduced_id(self) -> None:
        ...
        self.assertEqual(
            review["analysis_census"],
            {
                ...
                "string_sink_decoy_count": 65,
                "string_sink_decoy_sha256": (
                    "bddae53a4bf7a91561df99073de3813df6a6e925ce3c66704cf6af921c299bfe"
                ),
        ...
        self.assertIn(source_seal_id, expanded_items)
        self.assertTrue(
            any(
                row["item_id"] == source_seal_id
...
class CheckedInProfileTests(unittest.TestCase):
    ...
    def test_gitattributes_pins_all_three_governance_files_to_lf(self) -> None:
        ...
        original = attributes_path.read_bytes()
        with tempfile.TemporaryDirectory(
            ...
            _, _, identity = generator._strict_git_snapshot(git)
            lease = generator._acquire_git_launch_lease(git, identity)
            try:
                ...
                attributes_path.write_bytes(original)
                lease.close()
