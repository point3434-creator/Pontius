...
def _unittest_condition_kind(node: ast.expr) -> str | None:
    if not (
        isinstance(node, ast.Call)
        and isinstance(node.func, ast.Attribute)
        and isinstance(node.func.value, ast.Name)
...
def _decorator_platforms(
    ...
    for decorator in decorators:
        condition_kind = _unittest_condition_kind(decorator)
...
def discover_test_sources(sources: Mapping[str, bytes]) -> Discovery:
    ...
    for relative_path in sorted(normalized_sources):
        ...
        for class_node in (node for node in tree.body if isinstance(node, ast.ClassDef)):
            ...
            class_platforms = _decorator_platforms(class_node.decorator_list)
            ...
            for node in class_node.body:
                ...
                if not node.name.startswith("test_"):
                    continue
                method_platforms = _decorator_platforms(node.decorator_list)
...
def _posix_executable_snapshot(raw: bytes, mode: int) -> tuple[int, Path]:
    ...
    try:
        if hasattr(os, "memfd_create"):
            ...
            writable, temporary_path = tempfile.mkstemp(
                prefix="pontius-git-snapshot-",
            )
        offset = 0
    ...
    if windows:
        if expected_identity is not None:
            ...
            _windows_replace_governance_file(
                destination_path,
                destination_path.with_name(temporary_name),
...
def _write_posix_governance(
    ...
    if (
        ...
        try:
            ...
            if publish_state.posix_exchange:
                exchanged = _posix_exchange_governance_file(
                    directory,
                    path.name,
    ...
    if publish_state.displaced and not publish_state.published:
        try:
            ...
            cleanup_failures.append(error)
    if (
        publish_state.recovery_name is not None
        and not publish_state.displaced
        and not publish_state.published
        and outcome.recovery_path is not None
    ):
        try:
            os.unlink(publish_state.recovery_name, dir_fd=directory)
...
def _write_windows_governance(
    ...
    if (
        ...
        try:
            if publish_state.recovery_deletion_armed:
                _windows_cancel_relative_disposition(
                    publish_state.destination_handle
                )
                publish_state.recovery_deletion_armed = False
            _windows_rename_governance_file(
    ...
    if temporary is not None:
        try:
            if temporary.state == "closed_path" and not temporary.renamed:
                if staged_snapshot is None:
                    raise InventoryError(
                        "governance staging identity is absent during cleanup"
                    )
                temporary.handle = _windows_open_relative_governance_file(
                    directory,
                    temporary.name,
                )
                if _windows_governance_handle_file_id(
                    ...
                    raise InventoryError(
                        "governance staging cleanup refused changed identity"
                    )
                temporary.state = "open"
...
def _environment_delta(
    ...
    for call in execution.calls:
        if not (
            ...
            continue
        key = _static_value(call.args[0], assignments)
...
def _process_definition(
    ...
    *,
    ...
    environment, removals, environment_error = _environment_delta(
        keywords.get("env"), assignments, execution
    )
...
def _call_definition(
    ...
    elif qualified in _SCIENTIFIC_PROTECTED_CALLS:
        kind = "scientific"
        module_name = qualified.rsplit(".", 1)[0]
        return_contract = "opaque"
