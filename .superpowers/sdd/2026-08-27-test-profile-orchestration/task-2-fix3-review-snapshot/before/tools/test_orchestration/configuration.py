...
def _validated_ancestor_chain(path: Path, root: Path) -> tuple[tuple[Path, tuple[int, ...]], ...]:
    ...
    while True:
        ...
        chain.append((candidate, _directory_identity(info)))
...
def _capture_repository_root(repository_root: Path) -> Path:
...
def _read_bounded_snapshot(
    ...
    *,
    ...
    try:
        ancestors = _validated_ancestor_chain(supplied, root)
...
def _parse_inventory(
    ...
    actual_baseline = {
        "test_file_count": len({
            ...
            for item in entries
            if item.selector.stable_id in set(baseline_ids)
        }),
...
def load_configuration(
    ...
    *,
    ...
    try:
        root = _capture_repository_root(repository_root)
        profile_snapshot = _read_bounded_snapshot(
            profiles_path, root=root, label="profiles"
        )
        inventory_snapshot = _read_bounded_snapshot(
            inventory_path, root=root, label="inventory"
        )
