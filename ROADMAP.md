...
- **Response semantics:** add responder raises first at small private width.
  ...
  ADR-0377 now retains that one invocation: all six stages and all 126 gates
  pass, pool use returns to zero, and 40 cards reaches a 10.046424 GB requested
  peak with 15.710814 GB live free. The 327.622 ms warm and 296.192 ms source-
  refresh medians are measured lane units; the 49,557.238 ms direct scan is an
  ...
  invocation as a passing exact repaired-byte instrument seam. ADR-0413 now
  ...
  source seal; population 25 and the real result remain absent. ADR-0420 now
  completes that seal after 18 controls, including a complete 86-observation
  synthetic rebind, fake complete/one-zero compiler paths, exact fused-slot
  work, and a synchronization-first host ledger with a distinct population
  elapsed wall, cross-family overlap rejection, and commandless-cleanup
  ...
  an independent byte-loop hash differential before source seal. ADR-0425 now
  completes that seal after 20 V2 and 18 unchanged V1 controls plus a scrubbed
  external-working-directory two-launcher probe. Invoke its root launcher once
  from the clean seal commit, retain the first terminal without retry, and
  independently rebind it before any new projection or population-25 work.
- **Capacity preflight:** in parallel with response semantics, price one
  ...
  was its one clean exclusive invocation. ADR-0377 retains the completed pass
  through 40 cards without extrapolation. The immediate checkpoint is now a
  ...
  absent. ADR-0425 completes the new-identity launcher-safe source seal under
  ADR-0422's root-launcher contract as corrected by ADR-0423's lifecycle
  allowlist and ADR-0424's parent hashes. The immediate capacity-lane
  checkpoint is its one clean no-argument root-launcher invocation and
  independent retained-outcome read; it neither reopens V4 nor selects a new
  projection rule.
