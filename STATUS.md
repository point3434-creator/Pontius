...
Retain the first terminal exactly as `population_scientific_rejection`. V2 ran
once from clean source-seal commit `db01621` and is permanently consumed. The
artifact is complete, hash-bound, and independently rebound through the sealed
V2 lifecycle reader and unchanged V1 semantic oracle. It may not be retried,
repaired in place, resumed, skipped past the failure, or used as a favorable
partial population result.

The result is a real invocation and a deterministic evidence-construction
rejection. It is not a package-resolution, bootstrap, compiler, container,
module-load, resource-inspector, complete-ten, query-weight, timeout, cleanup,
or journal-integrity failure. It also is not a complete population-10 numerical
result: no population summary row survived the comparison, and population 22
never opened.

### Retained device boundary

The public root launcher reached its fresh parent and no-CUDA bootstrap child.
The bootstrap passed and the journal binds the clean source commit. The exact
shared CUDA source compiled into the retained one-zero-completed container;
the identical loaded cubin and raw resource evidence rebound successfully.
The primitive and complete-ten retained-cubin differential passed, as did the
query-weight control. Both ordered population-10 families emitted all fifteen
phase names through `final_release`; 2,768 phase rows were retained.

Those facts establish that the launcher repair worked and that the campaign
crossed its previously blocked compiler/resource/device seams. They do not
establish a complete population numerical verdict because the population
evidence object was never emitted.

### Exact failure and independent diagnosis

After both families released, `_population_evidence` called ADR-0394's
`_pair_error_evidence`. Its first source authority is generated by ADR-0394
`sample_rows`, which freezes sixteen source ranks and sixteen labeled query
records per calibration population. The generated execution runner did not
capture that helper. `build_generated_population_runner` copied
`vars(legal_river_quotient_cuda_compensated_tiles)`, so the generated
`_run_device_population` resolved the older parent `_sample_rows`: at
population 10 it selected seven source ranks and seven query records.

The source comparison therefore flattened seven rows times eight boundary
features into 56 actual pairs while constructing sixteen rows times eight
features, or 128 expected pairs. `_maximum_pair_errors` rejected before reading
a value. A source-only adversarial reproduction inspected the compiled
function's globals and proved the bound helper is exactly the parent helper,
not the evidence helper, with lengths `(7, 7)` versus `(16, 16)`. This matches
the retained exception and is independent of numerical values.

This is a plumbing/schema defect: two same-shaped callable concepts named
`sample_rows` carried different frozen populations. It is another semantic
quantity hidden behind a compatible Python signature. The structural repair is
one immutable sample-plan object consumed by execution and evidence, with
identity and cardinality checked before device work—not merely changing one
dictionary assignment.

### Successor boundary

Any successor receives entirely fresh lifecycle and result identities. Before
source, it must freeze the exact population-keyed source ranks, query records,
boundary features, and four comparison shapes. The source seal must prove the
generated runner and evidence calculator receive the same object; inspect the
generated globals; fail the historical 7/16 mutation before CuPy; and cover
both populations 10 and 22. The evidence error must name the comparison label
and actual/expected shape if a future mismatch occurs.

The compiler/container/resource, complete-ten, family order, phase/work,
numerical, population-wall, population-25 prohibition, null projection, and
claims contracts stay unchanged. V2's completed device work is diagnostic
evidence for this failure only and cannot be spliced into a successor.
...
Do not rerun or repair V2. Prospectively preregister one fresh sample-plan binding successor before source: compile one immutable population-keyed sample plan, inject that exact object into both the generated execution namespace and evidence authority, assert helper identity plus source/query rank and expected-shape equality before any CuPy import or device work, add the seven-versus-sixteen regression as a mandatory fail-closed control, retain the unchanged ADR-0419 compiler/resource/complete-ten/10-then-22 science and claims boundaries, and require a new launcher, protocol, campaign, header, result path, independent reader, source seal, and one-shot invocation
...
Latest process decision: [ADR-0426](docs/decisions/ADR-0426-retain-the-v2-population-sample-plan-rejection.md) — Retain the V2 population sample-plan rejection.
...
- Latest ADR: [ADR-0426](docs/decisions/ADR-0426-retain-the-v2-population-sample-plan-rejection.md) — Retain the V2 population sample-plan rejection.
- Governing runtime contract: [ADR-0307](docs/decisions/ADR-0307-make-action-clock-and-preparation-bank-authoritative.md) — Make the action clock and preparation bank authoritative.
- Numbered decisions: 426.
- ADR-header SHA-256: `07838e1bec748a5a13e847283727272de6373b47ef7b6b8d0e14db11cee09549`.
- Current blockers: the sole V2 path is consumed with a population-10 sample-plan plumbing rejection; no prospectively frozen sample-plan successor or complete 10/22 shared-path result exists; no replacement capacity estimator or projection is selected, and no complete 25-card numerical result, actual 45-card owner, or full-width actual-context quotient value exists; no general odd-chip or side-pot leaf automaton, repeated-actor multiway existence result, off-tree opponent-action result, cross-street belief and certificate handoff, certified full-width river strategy bridge, sealed blueprint trainer/checkpoint/abstraction/slice-audit chain, trained blueprint, v0a/v0b integrated bot, preparation-bank filling result, frozen evaluation opponent pool, complete 15-second decision, production action width, or poker-strength result exists.
...
4. [ADR-0426](docs/decisions/ADR-0426-retain-the-v2-population-sample-plan-rejection.md), [ADR-0280](docs/decisions/ADR-0280-exact-pre-bet-row-cache-passes-cpu-h2-fail-closed-control.md), [ADR-0307](docs/decisions/ADR-0307-make-action-clock-and-preparation-bank-authoritative.md), and their dependencies
