...
existing decision provider, and compare the provider's emitted decisions against
the teacher's own policy over a declared enumeration of the teacher's reachable
information sets. Unsupported states and fallback are counted and reported,
...
teacher's own policy for decision agreement. That last one is stated plainly because it is the weak point:
...
1. Every information set the teacher defines either resolves through the provider
   to the same action distribution within a declared tolerance, or is reported
   unsupported. No third outcome exists.
2. Decision agreement is reported over a declared enumeration, with unsupported
   and fallback counts stated separately from disagreements.
...
5. Every compared policy plays the identical `(hero hand, villain hand, board)`
   triple, and the hand-swap triple is also played. The matched pair, not the
   hand, is the unit of analysis. Seat rotation is unavailable: the teacher
   game's root exists only with the hero in the big blind.
6. Boards are the sampling clusters. A single-board result is conditional on
   that board and says nothing about others; Slice B runs four declared boards
   and reports per-board and pooled, stating the weakness of few-cluster
   inference rather than hiding it.
7. A loss budget is declared before the holdout run. A result inside its interval
   is reported **inconclusive**, never equivalent.
...
pass showed the originals unsatisfiable against the actual key space; the
[design](design.md) records why and what was accepted.
