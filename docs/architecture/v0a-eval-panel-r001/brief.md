...
1. Every information set the teacher defines either resolves through the
   ...
   The observable is the runtime's v1 `DecisionRecord` in `blueprint-v1` host
   mode (`selection_reason` of `table_hit` or `passive_default`, with
   `selected_action`), since that mode does not instantiate the provider class;
   the `BlueprintProvider` public boundary is checked separately and directly.
   No third outcome exists on either path.
2. Decision agreement is reported over a declared enumeration as hits,
   disagreements, unsupported, and excluded (interrupted timing, non-null
   failure, or unaccepted delivery), each counted separately.
...
5. Every compared policy plays the identical `(hero hand, villain hand, board)`
   ...
   the unit of analysis, and an excluded cell excludes its whole unit. Seat
   rotation is unavailable: the teacher game's root exists only with the hero
   in the big blind.
...
pass showed the originals unsatisfiable against the actual key space, and
criteria 1, 2, 5 and 6 were refined the same day after the r001 cold review;
the [design](design.md) records why and what was accepted.
