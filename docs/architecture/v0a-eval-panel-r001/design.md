...
1–2 (no third outcome; agreement over a declared enumeration) — observed at
two boundaries, failure first. The **hand outcome** comes from the envelopes
the session actually retains: the hand entry's `status` and `failure_reason`,
the host `hand_result` (`complete`, `settlement`, `interrupted_response_count`,
`accounting_complete`, `failure_reason`), and each `event_result` frame
(`status ∈ {accepted, decided, failed}` with independently nullable `decision`
and `failure`). A failed publication raises before any decision record exists
and arrives as `decision = None` with a `FailureRecord` carrying
`delivery_status`; the v1 `DecisionRecord` itself has **no** delivery field.
Only a completed, settled hand is then classified by its river
`DecisionRecord` — `selection_reason` exactly `table_hit` or `passive_default`,
plus `selected_action` — read from the retained child frame stream
(`child_stdout_base64`, with `capture_truncated` as an exclusion). The host
does **not** instantiate `BlueprintProvider` in `blueprint-v1` mode
(`runtime.py` sets `_provider = None`), so provider labels never appear there;
a separate direct check exercises the `BlueprintProvider` boundary against the
frozen teacher. Neither boundary is inferred from the other.
...
5. **An outcome classifier with failure precedence, then the river record,
   and separately the provider boundary.** For every hand in `H` and a seeded
   villain hand, one host session is played in `blueprint-v1` mode. The
   classifier runs in this order and stops at the first match:
   (a) the session hand entry has `status ≠ completed`, a non-null
   `failure_reason`, or `capture_truncated = true` → **excluded**. This is the
   outermost envelope and subsumes every host `child_failed`: an
   `event_result` with `status = failed` (whose `decision` is `None` in this
   mode and whose `failure` carries `code`, `delivery_status`, and nullable
   `timing`), an incomplete `hand_result`, a `state_mismatch`, a protocol or
   cleanup failure. Failures that arrive with no decision record are therefore
   classified without ever needing one.
   (b) The hand is completed and settled. Decode the retained child frame
   stream; there must be exactly one `event_result` whose `decision` has
   `street = river` and the controlled seat — zero or more than one →
   **excluded** with a protocol diagnostic. The host has already enforced that
   this record's `timing` is completed with no cutoff, its `failure_reason` is
   null, and its `selected_action` equals the action the host applied.
   (c) Classify: `selection_reason = table_hit` with `selected_action` equal to
   the teacher's action → **hit**; `table_hit` with another action →
   **disagreement**; `passive_default` with `h ∉ H` → **unsupported**;
   `passive_default` with `h ∈ H` → **disagreement** (the table has the key
   but the lookup missed: a key-identity fault). Pre-river decisions of the
   same hand must all be `passive_default`; any other pre-river reason →
   **disagreement**. In every case the panel also computes its own
   `PreparedBlueprint.action_for` on the replayed key and requires its
   `table_hit` flag to equal the record's `selection_reason` — the v1 host
   performs no independent lookup, so this is the only cross-check on the
   reason itself, and it is what distinguishes a hit from a default when the
   teacher's action is `check`, where the applied action alone cannot.
   Separately, the `BlueprintProvider` public `propose` boundary is driven
   directly with constructed observations for the same hands, and its
   `blueprint_hit`/`blueprint_default` proposal reasons are checked against
   the same expectations; the coverage record states which case proves runtime
   behavior and which proves provider behavior. Invariant: hand outcome
   before decision; the v1 record is the oracle for criteria 1–2 only on a
   completed hand; counts are reported as hits, disagreements, unsupported,
   excluded, with the exclusion reason.
...
7. **One inclusion law, pairing, and labels.** Acquisition rejects a deal only
   ...
   private-card draws. Two labels appear in the run record with counts, and
   they do different things. **Prefix-diverged** — a policy acted pre-river so
   the river root was never reached; `baseline-rules-v1` does this with
   premium hands, so it is card-dependent by construction — is a *label, not an
   exclusion* for chip comparisons: the hand still completes with a settlement,
   and that settlement is the deployed policy's outcome, pre-river behavior
   included. It excludes the cell only from the Slice A agreement enumeration,
   which needs the root. Deleting diverged units would filter the analysis
   population by the cards, which is exactly the selection r001 was faulted for.
   **Cutoff or failure** — any hand whose outcome is not a completed hand with
   accepted delivery, as classified at the outcome boundary in mechanism 5 —
   is the only exclusion. An excluded cell excludes its **whole unit across all
   policies and both orientations**, and every such exclusion has the
   inferential consequence stated in mechanism 9; pairing integrity is
   preserved, sampling validity is not assumed. Board order is declared
   ascending and used identically in export and session composition, since the
   key copies the card view's board order.
...
9. **Estimand, strata, interval, and sample size.** The estimand is the mean
   ...
   bounded range by a Hoeffding-type bound on the *planned* units per board,
   `n ≥ (8s)² · ln(2/α) / (2ε²)` with `α` split across the four boards, and it
   is not loosened after any holdout observation. **Missing units change what
   may be claimed.** Deleting a unit for cutoff or failure yields survivors
   whose mean targets `E[D | every cell survived]`, and nothing in this design
   makes survival independent of `D` — a cutoff that fires when the hero would
   bet is admissible in the frozen runtime. Therefore: if zero units on a board
   are excluded, the board's full-population result stands. If any are, the
   full-population acceptance or ranking claim for that board — and for any
   pooled result that includes it — is **withheld and reported inconclusive**,
   with the diagnostic counts, the survivors' mean and interval, and Manski
   worst-case bounds that place each missing unit at `−4s` and at `+4s` over
   the planned denominator. Meeting the floor with survivors does not restore
   the claim; only re-running the excluded units to completion does. A
   survivor-population estimand is not adopted; adopting one would be a new
   ruling with its own calibration. **Normalization** is written once so a
   factor of two cannot slip: if `m_P,B` is a policy's exact expected net chips
   per orientation on board `B`, the expected matched-unit total is `2·m_P,B`
   and the expected unit difference between policies `A` and `C` is
   `2·(m_A,B − m_C,B)`; calibration compares the empirical mean of matched-unit
   totals with `2·m_P,B`, and every pooled quantity, empirical or exact, uses
   the same fixed `1/4` board weights. The same definitions govern calibration,
...
`v0a/runtime` and `v0a/model` (the v1 `DecisionRecord` and `FailureRecord`),
`v0a/trace` (the serializer that keeps those two shapes distinct on the wire),
`legal_decision_spine_v2` (public-state digest and spine-reason contract used by
runtime, host, and the continuation), the `decision_provider`
model/providers/selection/codec, `tools/v0a_event_adapter` (the `event_result`
frame with nullable `decision` and `failure`), `tools/v0a_table_host`
(`select_opponent`, `TableInput`, `hand_result`, settlement),
`tools/v0a_table_session` (hand entry `status`/`failure_reason`),
`tools/v0a_seeded_deals`, and `evaluation`. Their base blobs are pinned in the
round's coverage record. The freeze's whole-tree diff against base is the
identity guard; the inventory is the semantic one — and r002 missed a
delivery-field error precisely because the serializer was not on it.

Slice A tests: key identity through the host for a sample of `H` and off-pool
hands, read from the river `DecisionRecord` in the retained frame stream; the
independent `action_for` cross-check of `selection_reason`, including an
in-pool hand whose teacher action is `check` (hit and default coincide on the
applied action and must be separated by the reason); the direct
`BlueprintProvider` boundary check for the same hands; set equality of the
unsupported set; wire-byte capacity at the chosen |H| with the conservative
placeholder and the final-artifact recheck; T1 purity and export determinism
(two exports of one frozen policy are byte-identical); the per-hand T1 action
equal to `best_response` on the sealed singleton-hero game for a declared
sample, including a hand whose check and bet values tie; the cost preflight's
recorded numbers; a deliberately perturbed prefix (one different stack) must
produce zero hits — the discriminating control that shows the identity check
has teeth; and a board supplied in non-ascending order must be refused or
canonicalized identically in export and composition.

Classifier schedules, using the frozen payload shapes and never an invented
field: a successful v1 `event_result`; a failed `event_result` with
`decision = None` and a `failure` whose `delivery_status` is `rejected`, and
another `unknown`, each before any river record; an accepted delivery followed
by a timing failure on a later event; a `hand_result` with `complete = false`
and `settlement = None`; a session hand entry with `capture_truncated = true`;
a completed hand with zero river records and one with two. Each must land in
**excluded** with its reason, and none may reach step (c).
...
excluded; a prefix-diverged `baseline-rules-v1` cell that is labeled and
**retained** in the chip comparison while excluded from agreement;
settlement-to-net-chips arithmetic against the kernel's `net_returns`; the
exact-expectation reconciliation of the deployed policy on a small pool where
the per-hand enumeration, `expected_utilities` on the sealed game, and the host
agree to the chip, with the `2·m_P,B` normalization applied; the equal-weight
stratified pooling arithmetic against a hand-computed example; and a
hand-calculated missingness case — exclusions correlated with a known nonzero
unit difference, enough survivors to meet the floor — in which the
full-population claim is withheld, the survivors' mean is reported as
conditional, and the Manski bounds over the planned denominator contain the
true value while the survivors' interval does not.
...
it reached the river root — or, the opposite error, *deleting* it, which
filters the population by the cards; r002 did that. Reading delivery status
off a `DecisionRecord`, which has none, and thereby never seeing the failures
that arrive with no decision record at all — r002 did that too. Pooling a
clock-induced fallback as a passive decision. Treating the survivors' mean as
the population mean once anything has been excluded. Sorting the board in one
place and not the other. Choosing `H` by strength. Adjusting `H`, `s`, or `k`
after seeing a holdout result. Reading the T1 calibration match as evidence
that T1 is good poker. Treating an observed sign as the direction gate.
Halving or doubling by mistake between per-orientation means and matched-unit
totals. One journal line per session instead of per run.
