> Historical ledger archived on 2026-09-08. Its findings and original wording
> are preserved below; relative links were adjusted for this location.
> Start with the [current results index](../../experiments/RESULTS.md) for the
> maintained conclusions. Historical workflow instructions are not current rules.

# Experiment results index

Start with a family summary to understand what we learned and what remains
unresolved. Open its evidence links when you need the measurements or method.

| What do you need to understand? | Start here | Consolidation status |
|---|---|---|
| Blueprint capacity, preparation, harness startup, and reuse | [Blueprint representation and runtime cost](../../experiments/blueprint-performance.md) | Pilot complete: current conclusions, comparable measurements, limitations, and reproduction links in one summary |
| h32 solver cost, GPU quotient checks, or compiled calibration | [Preserved family findings below](#cleanup-harvest--2026-09-08) | Findings harvested; family summaries still pending |
| Earlier search-quality and engineering results | [Historical entries below](#historical-entries--retained-from-the-earlier-ledger) | Historical narrative retained; consolidation pending |
| Whether a particular recent run completed | [Execution journal](../../execution_journal.jsonl) or [recent status](../../STATUS.md) | One journal entry per run; status is generated from it |

The blueprint pilot replaces this index's two former blueprint summaries. Its
supporting reports and raw artifacts remain in place. Other families remain
below until their own summaries are checked. A source commit recovers code;
ignored and machine-local measurements need separate retention.

## Cleanup harvest — 2026-09-08

The entries below summarize recorded historical results; this cleanup did not
rerun those experiments or independently revalidate their raw artifacts.
Historical process restrictions quoted in older entries are not current working
rules. The blueprint findings now live in the family summary linked above.
This harvest is partial; unlisted one-off runs still need review before deletion.

**Where does an h32 resident solver step spend its time?** Method: profile two
previously exposed timing-extreme targets with three exact restarts each,
recording 36 traverser rows. Result: combined device work accounted for 67.87%
of six-step time and host record-to-hand folding for 30.56%; transfer was 0.63%.
The pooled median complete step was 9,433.52 ms. A 2x device-only improvement
projected a 1.510x complete-step speedup under a serial model; that projection
was not a measured optimization or hardware comparison. Source commit:
`9f62521425236de73b1168fe9fa6d292b7e90900`. Recorded evidence:
[ADR-0198](ADR-0198-device-pipeline-dominates-h32-step-host-fold-is-second.md),
`experiments/results/h32-resident-step-bottleneck-profile-v2.json`.

**Does the shared-direct GPU quotient agree with the reference on its reduced
validation populations?** Method: run the V3 shared-sample-plan differential
on complete populations 10 and 22, including arithmetic, query/fold, numerical,
work-count, resource, and release checks. Result: both populations passed, with
complete validation walls of 9.348 s and 49.419 s; maximum recorded kernel
resources were 48 registers and 1,024 bytes of stack-plus-local backing per
thread. These are campaign walls, not resolver-step or decision latency. No
population-25 or actual-45 value, capacity projection, or poker-strength result
was produced. Source commit: `00d29e048d88ff4ece325b2a00866ddbd084617c`.
Recorded evidence:
[ADR-0429](ADR-0429-retain-the-passing-shared-sample-plan-v3-validation.md),
`artifacts/work_preflight/legal_river_quotient_cuda_shared_direct_device_v3.jsonl`.

**Which fixed-width arithmetic arms pass reduced device checks and the symbolic
45-card memory bound?** Method: compare positional, batched-five-then-four RRNS,
and resident-nine RRNS on complete-10 and signed-12, one warmup plus six timed
repetitions per arm and population. Result: all 42 observations passed exact
differentials. Positional and batched RRNS passed the symbolic memory bound;
resident-nine exceeded available memory after reserve by 1,319,284,904 bytes.
The full laboratory run took 19.672 s. The two eligible arms traded places
across the timing axes, so no candidate was selected; no actual-45 numerical
or action-latency result followed from this check. Source commit:
`f271e6deeefe11f228d582c9734a166c0ba32b45`. Recorded evidence:
[ADR-0448](ADR-0448-retain-the-passing-split-runtime-fixed-width-device-preflight.md),
`artifacts/work_preflight/legal_river_quotient_fixed_width_device_preflight_v3.jsonl`.

**Did compiled global-separation calibration establish a usable topology or
arithmetic schedule?** Method: run the V7 calibration with 480 warmup cells and
2,400 planned measured cells under a 1,500 s laboratory budget. Result: it
stopped at 1,510.054 s after warmup and 89 partial-pass measured-labelled cells;
no measured pass completed, no fit projection was emitted, and no candidate,
topology, or arithmetic schedule was selected. A recorded diagnostic attributed
1,247.207 s to base structural cover, 96.41% of recorded primitive time, but the
incomplete schedule and absent production base-refresh cadence do not support
an arm ranking or production speed estimate. Invocation commit:
`aaca2dda40e29be8ebd091d58e7853bce1c62fd8`. Recorded evidence:
[ADR-0476](ADR-0476-retain-the-v7-laboratory-wall-rejection.md),
`artifacts/work_preflight/legal_river_quotient_compiled_global_separation_calibration_v7.jsonl`.

The compiled-calibration v1–v7 launchers, experiment implementations, and their
result readers were removed after the calibration finding above was harvested.
The recorded V7 invocation commit retains the full family in Git history. The
reusable quotient, topology, and exact-integer operator modules remain. The h32
profile-v2 wrapper was also removed; its finding and reproduction commit remain
above. This does not mark the other one-off families as harvested.

**Why did the old native CI diagnostic fail on Python 3.11?** Method: compare
Windows directory identities from `os.stat` and `FileIdInfo`, then exercise the
old inventory writer and held-file cleanup. The recorded finding was a mismatch
between 32-bit stat volume serials on Python 3.11 and 64-bit handle identities;
the correction used full-width identities consistently. The archived report
records passing checks on 3.11.15 and 3.14.6 and hosted CI run `33293625754`,
while correcting older purported 3.11 evidence to 3.14.6. Source commit:
`b5df265cf04739cbd051e9de5423bb1e5a66508a`. Recorded evidence:
[ADR-0481](ADR-0481-record-the-release-interpreter-identity-correction.md).
This cleanup did not rerun that historical diagnostic. Its only dependency was
the now-deleted inventory generator, and it has no remaining CI caller, so
`tools/ci_native_diagnostics.py` was removed with that machinery.

## Historical entries — retained from the earlier ledger

The following narrative records the project state when each entry was written;
references to a "current" decision below are historical.

For the live decision, read generated [STATUS.md](../../STATUS.md). The current
engineering result is
[ADR-0237](ADR-0237-full-bisector-library-does-not-fit-every-street.md):
...
[ADR-0235](ADR-0235-one-step-retained-after-heldout-depth-value-trial.md):
...
[ADR-0190](ADR-0190-selector-stable-affine-certificate-is-exact-and-fits-retained-street-ledgers.md):
...
[ADR-0192](ADR-0192-fixed-seat0-affine-rule-emits-four-fresh-certified-candidates-before-deadline.md)
records the completed trial over the four remaining unseen seat-0 blocker
shifts. All four fixed regret vertices produce deadline-eligible simulated
emissions, with at least 3.80 seconds of boundary headroom and machine-precision
post-emission exact agreement. Total certified value is `1.1263e-6`, but one
...
[ADR-0194](ADR-0194-affine-street-mechanism-transfers-to-seat5-but-value-remains-concentrated.md)
records the direct other-extreme replication. Acting seat 5 also produces four
deadline-eligible exact emissions, but one target carries 91.50% of its value
and the target spread is 11,004x. Across both extreme-seat panels, two targets
...
[ADR-0199](ADR-0199-preregister-retained-affine-selector-cascade-replay.md)
...
[ADR-0201](ADR-0201-preregister-memory-schema-corrected-selector-replay.md)
then rejected before the warm step on an overstrict source-schema guard. The
final [ADR-0203](ADR-0203-preregister-final-four-field-selector-replay-correction.md)
...
[ADR-0205](ADR-0205-preregister-reviewed-direct-selector-replay.md)
