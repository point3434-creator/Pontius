"""External operator for the exact approved ADR-0510 single cost rehearsal."""
...
request_hash = '67af9c6fb677af40937c7640e391bdabcf7f0c27a0a261ac9efdbbf98ddf8359'
...
def run_capture(name, argv, show_progress=False):
    ...
    try:
        ...
        while process.poll() is None:
            ...
            if show_progress and time.monotonic_ns()-last_progress[0] >= 10000000000:
                completed_records = sum((output/f'u{i:03d}'/'result.json').is_file() for i in range(1,25))
                print(json.dumps(dict(progress='rehearsal', retained_unit_records=completed_records,
                    planned_units=24, parent_elapsed_seconds=round((time.monotonic_ns()-started)/1e9,1))), flush=True)
...
assert adoption['candidate'] == '4c7882752bd93e56a05dba513c035b02ca2f2325'
save(packet/'rehearsal-opportunity.json', dict(user_message='I approve', decision='ADR-0510',
...
try:
    ...
    for path in (python, git_exe, taskkill):
        regular(path)
    request = (packet/'files/docs/architecture/v0a-paired-prereg-r001/rehearsal-request.json').read_bytes()
    assert len(request) == 352 and sha(request) == request_hash
    ...
    argv = [str(python), '-B', '-P', 'tools/v0a_evaluation.py', '--request',
        'D:/Pontius/tmp/v0a-paired-rehearsal-run-001/request.json', '--output-root',
        'D:/Pontius/tmp/v0a-paired-rehearsal-run-001/output', '--evaluation-id',
        'pontius-v0a-evaluation-v1-correctness-rehearsal-001']
    launched = run_capture('wrapper', argv, True)
    assert verify_source() == {k:v for k,v in identity.items() if k != 'executables'}
    assert (root/'request.json').read_bytes() == request
    companion = (packet/'files/docs/architecture/v0a-paired-prereg-r001/preregistration.md').read_bytes()
    assert sha(companion) == '9c43144abfca8d0b998142930c43204c25ddf801808166e67d6d9fe4064cc541'
    ...
    for i in range(1,25):
        ...
        unit = dict(ordinal=i, result_sha256=None, state='missing', elapsed_ns_prefix=None)
        if path.is_file():
            regular(path)
            raw = path.read_bytes()
            value = json.loads(raw)
            unit.update({k:value.get(k) for k in ('ordinal','strategy','state','cleanup_complete',
            ...
            for capture in ('stdout', 'stderr'):
                capture_path = path.parent/(capture+'.bin')
                if capture_path.is_file():
                    regular(capture_path)
                    capture_raw = capture_path.read_bytes()
                    unit[capture+'_bytes'] = len(capture_raw)
                    unit[capture+'_sha256'] = sha(capture_raw)
        units.append(unit)
    files = []
    if output.is_dir():
    ...
    counts = json.loads((root/'reader-stdout.bin').read_bytes()) if reader['exit_code']==0 else None
    report = dict(standing='Non-evidentiary cost rehearsal only; no poker outcome use',
        source=identity, request_sha256=request_hash, adoption_commit=adoption['commit'],
        wrapper=launched, reader=reader, completion=counts, units=units,
        output_regular_files=len(files), output_retained_bytes=sum(f['bytes'] for f in files),
        max_unit_stdout_bytes=max((f['bytes'] for f in files if f['path'].endswith('/stdout.bin')), default=None),
        max_unit_stderr_bytes=max((f['bytes'] for f in files if f['path'].endswith('/stderr.bin')), default=None),
        source_checkout_tracked_bytes=identity['tracked_bytes'],
        parent_interval_note='Includes parent scheduling and up to one-second supervision polling delay',
        unmeasured=['peak disk','process-tree memory','per-action p95','new-deal scaling'],
        opportunity_consumed=True, automatic_retry_authorized=False, operating_closure_admitted=False)
    save(root/'cost-report.json', report)
    print(json.dumps(dict(status='RETAINED', wrapper_exit=launched['exit_code'],
        wrapper_seconds=launched['parent_elapsed_ns']/1e9, reader_exit=reader['exit_code'],
        completion=counts, output_files=len(files), output_bytes=report['output_retained_bytes'],
        report=str(root/'cost-report.json'))), flush=True)
except BaseException as error:
    record = dict(status='INCOMPLETE', error_type=type(error).__name__, reason=str(error),
        opportunity_consumed=True, automatic_retry_authorized=False, utc=utc(), root=str(root))
    save(packet/'rehearsal-incomplete.json', record)
