...
- [ ] Run floor focused controls then current; retain all failed executions. No
  ...
  This is the one focused session-suite invocation per candidate. Final acceptance
  and added-CI rehearsal each admit one more complete invocation on each version,
  giving at most 48 new control sessions per interpreter per source candidate.
  Failed invocations count; further live iteration consumes a permitted correction
  candidate. Pre-launch RED failures remain recorded with zero launched sessions.
...
- [ ] Verify complete raw B preservation outside scope and all added paths. Rehearse
  the four added CI blocks once locally on each interpreter; distinguish that from
  hosted CI evidence. The source contract counts the session block explicitly.
