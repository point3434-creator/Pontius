...
def build_inputs(controller, directory):
    ...
    for scenario_index, scenario in enumerate(SCENARIOS):
        for seat in (0, 3):
            for repetition in range(2):
                ...
                block_seed = hashlib.sha256(f"{SEED}:{case // 16}".encode()).hexdigest()
                raw = dealer.deal_for_hand(block_seed, case % 16)
                stacks = (
    ...
    design = dict(
        seed=SEED,
        deal_seed_rule="sha256(f'{seed}:{case // 16}'); hand index = case % 16",
        scenarios=SCENARIOS,
        cells=cells,
        expected=expected,
        artifacts=metadata,
...
def main():
    ...
    (run / "experiment.py").write_bytes(driver_bytes)
