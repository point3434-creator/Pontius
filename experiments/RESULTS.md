...
| What can the bot actually do, and what do game results mean? | [Bot integration and validation](bot-validation.md) | Native mixtures execute correctly. A fresh comparison passed 768 hands and 96 sessions but rejected both policy candidates: a transferable fallback improved average worst-case NashConv 37.62% while harming a safe case by 0.830 chips; simple prior pooling worsened its average worst case 17.39%. Coverage and range assumptions need better policy methods, not faster lookup. |
| What limits blueprint lookup and benchmarking? | [Blueprint representation and runtime cost](blueprint-performance.md) | History-key size limits capacity; repeated preparation is a measured opportunity. The 144-hand passive hit batch passed, followed by 48 active-action sessions in 20.674 s with 64 hits and 20 intentional diagnostic misses. Bets, re-raises, folds, all-ins, side pots and refunds matched predictions. Reliable tails, near-cap active actions and trained-policy strength remain open. |
