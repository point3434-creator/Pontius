...
def worker(plan, out):
    ...
    bindings(plan)
    write(out/'worker-complete.json', dict(complete=True, cases=16, lp_calls=32,
                                          bettor_trajectories=32, decisions=32))
...
def verify(plan, out):
    ...
    bindings(plan)
    assert read(out/'worker-complete.json') == dict(complete=True, cases=16, lp_calls=32,
                                                   bettor_trajectories=32, decisions=32)
    write(out/'audit.json', dict(passed=True, cases=16, replayed_updates=updates,
        proposed_certificates=32, incumbent_certificates=32, independent_gate_audits=32,
