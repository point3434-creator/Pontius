...
def prepare():
    ...
    m = monitor_module()
    start = perf_counter()
    source = t.d.b.PRIOR/'input-000.json'
    record = read(source)
    nodes = t.public_tree(record, 'baseline')
    ...
    reference = HISTORY/'river-tree-expansion-001/run/000-baseline.json'
    write(HERE/'reference.json', read(reference)['certificate'])
    arms = [
    ...
    plan = dict(name='river-cfr-comparison-002', frozen_at=datetime.now(timezone.utc).isoformat(),
                source_commit='6518b9d3af252c93f8a9dbdb13fb28adc2859b4c',
                context_checkout='D:/Pontius-worktrees/eval-runner-consolidation',
                authorization='User: Ok let\'s run the next', case=0, variant='baseline',
...
def verify(index):
    ...
    assert max(t.Q(cert['lower']),t.Q(reference['lower'])) <= min(
    ...
    write(HERE/'run'/f'{label}-certificate.json', dict(executing_pid=os.getpid(),
          ...
          retained_reference_overlap=True, quantized_probabilities=probs))
    bindings()
...
def run(expected):
    ...
    bindings()
    write(out/'receipt.json', dict(complete=True, workers=18, certificates=6,
