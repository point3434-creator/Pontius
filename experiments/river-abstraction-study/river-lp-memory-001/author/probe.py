...
def traced_worker(index):
    sources={str(p.resolve()):p.read_text().splitlines() for p in trace_sources()}
    ...
    with (OUT/(job['label']+'-trace.jsonl')).open('x',encoding='utf-8',newline='\n') as stream:
        def emit(frame,event):
            source=sources[frame.f_code.co_filename]
        ...
        def trace(frame,event,arg):
            if frame.f_code.co_filename not in sources or frame.f_code.co_name not in functions:
                return None
            if event=='line':
                # Skip terminal/block iteration to avoid per-entry instrumentation.
                text=sources[frame.f_code.co_filename][frame.f_lineno-1].strip()
...
def freeze():
    ...
    for p in sources+[Path(__file__)]: pins[str(p)]=e.d.digest(p)
...
def selftest():
    ...
    e.write(HERE/'monitor-check.json',dict(before=before,after=after,passed=True,
                                        sparse_bytes=buffers['matrix']['bytes']))
