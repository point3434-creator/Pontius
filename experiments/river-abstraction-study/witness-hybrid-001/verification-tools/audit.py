"""Independent scalar/rational audit of the retained combined-feature experiment."""
...
scale_data=read(OUT/'scales.json')
training_cases=read(Path(plan['training'])/'plan.json')['cases']
for seat in (0,1):
    for name,field,columns in [('raw','raw',11),('prediction','prediction',4)]:
        variances=[]
        for case in training_cases:
            h=read(Path(plan['diagnostic'])/('training-'+case['id']+'.json'))['seats'][seat]
            total=fsum(h['weights']); w=[v/total for v in h['weights']]
            centers=[fsum(w[i]*h[field][i][c] for i in range(96)) for c in range(columns)]
            variances.append(fsum(w[i]*fsum((h[field][i][c]-centers[c])**2
                for c in range(columns)) for i in range(96)))
        expected=sqrt(fsum(variances)/48)
        assert isclose(expected,scale_data[seat][name],rel_tol=1e-12,abs_tol=1e-12)
...
verification=dict(passed=True,cases=48,new_floor_identities=96,asymmetric_gaps=192,
    signed_comparisons=672,aggregate_records=checked,scales_independently_verified=4,
...
analysis=dict(mean_floors={m:mid(v) for m,v in summary['overall']['mean_floors'].items()},
    ...
    boards={b:dict(floors={m:mid(v) for m,v in s['mean_floors'].items()},
                   hybrid_deltas={m:mid(v) for m,v in s['comparisons']['hybrid'].items()})
            for b,s in summary['boards'].items()},
    regimes={name:dict(floors={m:mid(v) for m,v in s['mean_floors'].items()},
        hybrid_counts={m:c['counts'] for m,c in s['comparisons']['hybrid'].items()})
        for name,s in summary['regimes'].items()},
    lobo_hybrid={other:[mid(s['comparisons']['hybrid'][other]) for s in summary['leave_one_board_out'].values()]
                 for other in ('predicted_only','raw11','range_equity','range_response')},
    worker_seconds=read(OUT/'worker-receipt.json')['seconds'],
    worker_plus_parent_seconds=read(OUT/'receipt.json')['seconds'],scales=scale_data)
