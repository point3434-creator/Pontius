...
def audit(out):
    ...
    methods=('ordinal','sign_conditioned','ordinary_preference','range_response','oracle_ordinal','oracle_sign','oracle_clipped')
    ...
    def stats(rs,n):
        ...
        for m in methods[1:]:
            lo=[];hi=[];labels=[]
            for r in rs:
                a=r['solutions']['ordinal']['minimum_exploitability']
            ...
            if m in solvers:
                ds=[a-b for a,b in zip(vals['ordinal'],vals[m],strict=True)]
    ...
    flags=dict(ordinal_actual_gain=pot['sign_conditioned']['actual']['delta']<Q(-1e-10),
