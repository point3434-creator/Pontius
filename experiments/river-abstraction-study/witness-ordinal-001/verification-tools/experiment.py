...
import tracemalloc
from loader import ROOT,PRIOR,np,opt,base,budget,transfer,read,write,digest,pref,RegretBR,anchored_clusters,action_advantages
from loader import model,parent
import ordinal

HERE=Path(__file__).parent
SEED='witness-ordinal-001'
BETS=(5,10)
BANK=('ordinary_preference','range_response','range_equity')
SOLVERS=('ordinal','sign_conditioned','ordinary_preference','range_response')
METHODS=(*SOLVERS,'oracle_ordinal','oracle_sign','oracle_clipped')
...
def worker(plan,out):
    ...
    candidate=dict(models=models,training_files={filename('train',c,b):digest(out/filename('train',c,b))
        for c in plan['training_cases'] for b in BETS},fit_tasks=18,
    ...
    assert calls==1024 and digest(out/'candidate.json')==candidate_sha
    bindings(plan)
    write(out/'worker-complete.json',dict(complete=True,training_cells=64,evaluation_cells=64,
        lp_calls=1024,fit_tasks=18,trajectories=256,checkpoint_policies=768))
...
def stats(rows,n=50000):
    ...
    comparisons={}
    for m in METHODS[1:]:
        diffs=[base.difference(r['solutions']['ordinal']['minimum_exploitability'],
        ...
        if m in SOLVERS:
            ds=[next(p for p in r['records']['ordinal'] if p['iteration']==n)['full']['exploitability']-
...
def summarize(rows):
    ...
    flags=dict(ordinal_actual_gain=pot['sign_conditioned']['actual']['delta'] < -1e-10,
        ordinal_floor_gain=Q(pot['sign_conditioned']['floor']['upper_exact'])<0,
    ...
    flags['cross_bet_robustness']=all(v['comparisons'][m]['actual']['delta'] < -1e-10 and
        ...
        for m in ('sign_conditioned','ordinary_preference','range_response'))
...
def verify(plan,out):
    ...
    for case in plan['training_cases']:
        for bet in BETS:
            ...
            assert row==read(PRIOR/filename('train',case,bet))
    ...
    assert candidate['baseline_sha256']==digest(PRIOR/'candidate.json')
    ...
    assert (certs,policies)==(1408,768)
    ...
    write(out/'audit.json',dict(passed=True,certificates=certs,checkpoint_policies=policies,
        replayed_iterations=12800000,fit_tasks_reproduced=18,maximum_scalar_discrepancy=maximum,
