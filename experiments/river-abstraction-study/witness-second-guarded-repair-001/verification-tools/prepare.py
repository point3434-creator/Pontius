"""Verify retained initial states and freeze before creating any second repair."""
...
assert previous[e.PRIOR.name] == (
    'a6c40ae7d19219d7e499e5386c5bcb3e3e0bb0aa0db6495d6da12ebe933619c0')
for label, cwd, tests in [('new', here, ['test_second']),
    ('frozen', e.PRIOR/'verification-tools', ['test_gate', 'test_experiment'])]:
...
# Only reconstruct already-known first-step policies. No second-step proposal or score.
for i in (0, 12, 32, 60):
    ...
    assert record['exact_exploitability'] == row['selected_score']['exact_exploitability']
e.c.equities_for.cache_clear()
e.write(here/'preflight.json', dict(passed=True, new_checks=5, frozen_checks=10,
    first_step_policy_replays=4, second_step_proposals=0, prior_milestones=previous))
...
plan = dict(name=e.NAME, output=str(output), sources=sources,
    ...
    control_checkpoints=[10000, 20000], repaired_checkpoints=[1000, 10000],
    phase_timeout_seconds=900, python=sys.version, numpy=e.c.np.__version__,
    scipy=e.c.scipy.__version__, user_words_verbatim="Let's build and run",
...
e.write(here/'freeze.json', dict(plan_sha256=e.digest(here/'plan.json'),
    ...
    retained_invoked=False, lp_calls=256, solver_trajectories=128, gate_calls=128))
