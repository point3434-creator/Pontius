"""Successor one-size resident CFR through the validated v2 device fold."""
...
class DeviceFoldResidentLeafAdjointPublicTreeCFRV2(
    ResidentLeafAdjointPublicTreeCFR
):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if type(reach_tolerances) is not ResidentReachTolerances:
            raise TypeError("device-fold v2 CFR requires ResidentReachTolerances")
        self.reach_tolerances = reach_tolerances
    ...
    def step(self) -> None:
        ...
        for traverser in range(self.num_players):
            ...
            result = device_fold_resident_leaf_adjoint_cfr_traverser_v2(
                self.layout,
                self.workspace,
                self.sparse,
                probabilities,
...
def device_fold_resident_leaf_adjoint_cfr_traverser_v2(
