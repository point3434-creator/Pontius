"""Selector-stable affine responses through the validated v2 device fold."""
...
from .convex_retreat_tolerances import (
    InterceptIdentityAllowance,
    SelectorMarginAllowance,
)
from .device_fold_resident_heterogeneous_leaf_contraction_v2 import (
...
def _semantic_row(
    ...
    *,
    ...
    selector_margin_allowance: SelectorMarginAllowance,
    intercept_identity_allowance: InterceptIdentityAllowance,
    affected_terms: int,
    terminal_contraction_ms: float,
    ...
    affine = _selector_stable_affine_reverse(
        ...
        selector_margin_allowance=selector_margin_allowance.absolute_margin,
    )
    ...
    if identity_error > intercept_identity_allowance.absolute_error:
...
def evaluate_device_fold_selector_stable_affine_leaf_adjoint_seat_v2(
    cache: LeafAdjointResponseSeatCache,
    ...
    *,
    ...
    if type(selector_margin_allowance) is not SelectorMarginAllowance:
        raise TypeError("device-fold selector requires SelectorMarginAllowance")
    if type(intercept_identity_allowance) is not InterceptIdentityAllowance:
        raise TypeError("device-fold selector requires InterceptIdentityAllowance")
    if type(reach_tolerances) is not ResidentReachTolerances:
        raise TypeError("device-fold selector requires ResidentReachTolerances")
    ...
    if terms:
        ...
        contraction = contract_device_fold_resident_heterogeneous_leaf_terms_v2(
            cache.workspace,
            cache.sparse,
            terms,
            ...
            payoff_span=payoff_span(cache.layout),
    ...
    semantic = _semantic_row(
        ...
        intercept_identity_allowance=intercept_identity_allowance,
...
def evaluate_device_fold_batched_selector_stable_affine_opponents_v2(
    cache: LeafAdjointResponseSeatCache,
