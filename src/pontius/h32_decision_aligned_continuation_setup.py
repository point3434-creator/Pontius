"""Decision-aligned continuation setup for the sealed h32 convex engine."""
...
def build_decision_aligned_continuation_setup(
    parsed: Mapping[str, Any],
    source_parent: Mapping[str, Any],
    spec: Mapping[str, Any],
) -> dict[str, Any]:
    """Reconstruct one pinned post-call posterior and its current-decision root."""
    ...
    observations = observed_bet_then_one_call_sequence(int(spec["observed_bettor"]))
    ...
    if public_prefix != expected_prefix:
        raise ValueError("decision-aligned public prefix differs from sealed manifest")
    if int(spec["observed_responder"]) != public_prefix[-1][0]:
        raise ValueError("decision-aligned observed responder differs from prefix")
    if str(spec["observed_response"]) != public_prefix[-1][1]:
        raise ValueError("decision-aligned observed action differs from prefix")
    ...
    if root_state.current_player != acting_player:
        raise ValueError("decision-aligned setup actor is not current at the root")
    if root_state.legal_actions() != ("fold", "call"):
        raise ValueError("decision-aligned setup root is not a call-fold decision")
    if len(root_state.pending_responders) != 4:
        raise ValueError("decision-aligned setup lost downstream response geometry")
...
@contextmanager
def decision_aligned_core_setup_adapter() -> Iterator[None]:
    """Temporarily route the sealed campaign core through the post-call setup."""
    ...
    core._setup = build_decision_aligned_continuation_setup
    try:
        yield
    finally:
