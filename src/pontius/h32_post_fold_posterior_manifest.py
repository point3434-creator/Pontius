"""Freeze fresh h32 post-fold posteriors at the next player's decision."""
...
_CONFIG = _ROOT / "experiments/configs/h32-post-fold-posterior-manifest-v1.json"
_OUTPUT = _ROOT / "experiments/results/h32-post-fold-posterior-manifest-v1.json"
...
_POST_CALL_DECISION = (
    _ROOT
    / "docs/decisions/ADR-0264-current-decision-convex-shadow-delivers-six-safe-candidates-in-five-seconds.md"
)
_IMPLEMENTATION = Path(__file__)
_TEST = _ROOT / "tests/test_h32_post_fold_posterior_manifest.py"

_PATHS = {
    "expected_source_result_sha256": _SOURCE,
    "expected_post_call_manifest_sha256": _POST_CALL_MANIFEST,
    "expected_post_call_result_sha256": _POST_CALL_RESULT,
    "expected_post_call_decision_sha256": _POST_CALL_DECISION,
    "expected_posterior_primitive_sha256": (
        _ROOT / "src/pontius/h32_decision_aligned_posterior_manifest.py"
    ),
...
_TARGET_PLAN = tuple(
    {
        "target_id": (
            f"{source}/checks_then_bet_seat{source_index}_then_"
            f"fold_seat{(source_index + 1) % 6}"
        ),
        "source": source,
        "observed_bettor": source_index,
        "observed_responder": (source_index + 1) % 6,
        "observed_response": "fold",
        "acting_player": (source_index + 2) % 6,
        "round": "decision_aligned_fold_v1",
    }
...
def _sha256(path: Path) -> str:
    if not path.is_file():
        raise ValueError(f"required post-fold manifest input is unavailable: {path}")
    return hashlib.sha256(path.read_bytes()).hexdigest()


def observed_bet_then_one_fold_sequence(bettor: int) -> tuple[dict[str, Any], ...]:
    """Return checks, one bet, and exactly the first responder's fold."""
...
def parse_h32_post_fold_posterior_manifest_config(
    config: dict[str, Any],
) -> dict[str, Any]:
    expected = {
        "evidence_stage",
    ...
    if set(config) != expected:
        raise ValueError("post-fold manifest fields differ from ADR-0265")
    for field_name, path in _PATHS.items():
        if config[field_name] != _sha256(path):
            raise ValueError(f"post-fold provenance mismatch: {field_name}")
    exact = {
        "evidence_stage": (
            "preregistered_after_adr0264_before_any_post_fold_warm_step_convex_"
            "candidate_certificate_or_strategy_label"
        ),
        "freshness_base_commit": "c46a622267b0581e2b0c67685a149a84f7446267",
        "seed": 20260822,
        "source_specs": [
        ...
        "panel_design": (
            "same_six_sources_bettors_responders_and_current_actors_as_post_call_"
            "with_only_the_first_observed_response_changed_to_fold"
        ),
        "observation_rule": (
        ...
        "acting_player_rule": (
            "second_responder_is_current_player_immediately_after_the_observed_fold"
        ),
        "topology_rule": (
    ...
    for field_name, expected_value in exact.items():
        if config[field_name] != expected_value:
            raise ValueError(f"post-fold field differs from ADR-0265: {field_name}")
    gates = {
        "expected_sources": 6,
        "expected_targets": 6,
        ...
        "require_post_call_manifest_passed": True,
        "require_post_call_parent_passed": True,
        "require_post_fold_authorized": True,
        "require_source_checkpoint_identity": True,
        "require_blueprint_identity": True,
        "require_disjoint_from_post_call": True,
    ...
    if config["gates"] != gates:
        raise ValueError("post-fold manifest gates differ from ADR-0265")
    return {
        **config,
...
def run_h32_post_fold_posterior_manifest(
    ...
    if git["dirty"]:
        raise RuntimeError("post-fold manifest requires a clean Git state")
    source_parent = load_artifact(
        _SOURCE,
        expected_sha256=parsed["expected_source_result_sha256"],
        require_passed=True,
    ).payload
    post_call_manifest = load_artifact(
        _POST_CALL_MANIFEST,
        expected_sha256=parsed["expected_post_call_manifest_sha256"],
        require_passed=True,
    ).payload
    post_call_result = load_artifact(
        _POST_CALL_RESULT,
        expected_sha256=parsed["expected_post_call_result_sha256"],
        require_passed=True,
    ).payload
    post_call_ids = {row["target_id"] for row in post_call_manifest["target_rows"]}
    post_call_digests = {
        row["target_belief_sha256"] for row in post_call_manifest["target_rows"]
    }
    ...
    for target_spec in parsed["target_plan"]:
        ...
        responder = int(target_spec["observed_responder"])
        ...
        schema = information_schema_for_axes(continuation, posterior.hands_by_player)
        ...
        row = {
            ...
            **target_spec,
            ...
            "observed_responder_marginal_total_variation": tvs[responder],
    ...
    responder_counts = Counter(int(row["observed_responder"]) for row in target_rows)
    ...
    checks = {
        ...
        "post_call_manifest_passed": artifact_passed(post_call_manifest)
        == gate["require_post_call_manifest_passed"],
        "post_call_parent_passed": artifact_passed(post_call_result)
        == gate["require_post_call_parent_passed"],
        "post_fold_authorized": (
            post_call_result["decision"]
            == "accept_decision_aligned_shadow_transfer_and_authorize_post_fold_preregistration"
        )
        == gate["require_post_fold_authorized"],
        ...
        "disjoint_from_post_call": all(
            row["target_id"] not in post_call_ids
            and row["target_belief_sha256"] not in post_call_digests
            for row in target_rows
        )
        == gate["require_disjoint_from_post_call"],
        "target_id_fresh": all(
    ...
    result = {
        "schema_version": 1,
        "status": "h32_post_fold_posterior_manifest_executed",
        ...
        **gate_result,
        "decision": (
            ...
            else "reject_post_fold_live_shadow_panel"
        ),
        "strategy_population_claim": None,
        "total_seconds": total_seconds,
        "limitations": [
            "The boards and source blueprints are retained; freshness applies to post-fold posterior identities.",
            "Every target observes a fold, so this panel complements but does not enlarge the board sample.",
            "The fixed position-balanced panel is not an IID sample of deployment decisions.",
            "Marginal TV and topology are label-free descriptors, not strategy-opportunity labels.",
            "A passing manifest freezes identities but does not authorize opening post-fold strategy labels.",
...
def main() -> None:
    ...
    result = run_h32_post_fold_posterior_manifest(args.config, args.output)
    print(
        json.dumps(
            {
