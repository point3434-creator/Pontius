"""Launcher-safe exclusive V3 owner for ADR-0427."""
...
_V2_OUTPUT = _ROOT / V2_RESULT_RELATIVE_PATH
...
SCIENCE_ADAPTER_MODULE = (
    "pontius.legal_river_quotient_cuda_shared_direct_sample_plan"
)

_MODE_ENV = "PONTIUS_ADR0427_SHARED_DIRECT_CHILD_MODE"
_CHALLENGE_ENV = "PONTIUS_ADR0427_SHARED_DIRECT_CHALLENGE"
_PUBLIC_PROBE_ENV = "PONTIUS_ADR0427_PUBLIC_LAUNCHER_PROBE"
_HANDSHAKE_MODE = "handshake"
_PUBLIC_CHILD_MODE = "public_probe_child"
_CAMPAIGN_MODE = "campaign"
_EVENT_PREFIX = b"PONTIUS_ADR0427_EVENT "
_ACK = b"ADR0427_ACK\n"
...
def canonical_lf_sha256(path: Path) -> str:
    if not isinstance(path, Path) or not path.is_file():
        raise ValueError(f"shared-direct V3 path is absent: {path}")
...
def load_preregistered_config() -> Mapping[str, object]:
    ...
    if canonical_lf_sha256(path) != CONFIG_SHA256:
        raise ValueError("shared-direct V3 config hash differs")
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ValueError("shared-direct V3 config root differs")
    return value


def verify_preregistered_contract() -> None:
    config = load_preregistered_config()
    retained = config.get("retained_v2_contract")
    ...
    if (
        ...
        raise ValueError("shared-direct V3 preregistered contract differs")
    for prefix in (
        "source_seal_adr",
        "outcome_adr",
        "config",
        ...
        if (
            ...
            raise ValueError(
                f"shared-direct V3 retained V2 parent differs: {prefix}"
            )
    if (
        retained.get("source_seal_commit")
        ...
        or _RESERVED.exists()
        or not _V2_OUTPUT.is_file()
        or _V2_OUTPUT.stat().st_size != V2_RESULT_BYTES
        or sha256(_V2_OUTPUT.read_bytes()).hexdigest() != V2_RESULT_SHA256
    ):
        raise ValueError("shared-direct V3 retained lifecycle differs")
...
def strict_git_metadata(*, result_created: bool) -> dict[str, object]:
    ...
    if len(commit) != 40:
        raise RuntimeError("shared-direct V3 Git commit identity differs")
    for relative in DEPENDENCY_RELATIVE_PATHS:
    ...
    if status != expected:
        raise RuntimeError("shared-direct V3 strict Git status differs")
    if (
        _V1_OUTPUT.exists()
        or _RESERVED.exists()
        or not _V2_OUTPUT.is_file()
        ...
        raise RuntimeError("shared-direct V3 protected result exists")
...
def _observation_payload(
    ...
    if not isinstance(kind, str) or not kind or not isinstance(event, Mapping):
        raise TypeError("shared-direct V3 child event differs")
    return {
...
def _child_emit(kind: str, payload: Mapping[str, object]) -> None:
    ...
    if len(line) > MAXIMUM_CHILD_LINE_BYTES:
        raise ValueError("shared-direct V3 child event exceeds line cap")
    sys.stdout.buffer.write(line)
    sys.stdout.buffer.flush()
    if sys.stdin.buffer.readline() != _ACK:
        raise RuntimeError("shared-direct V3 child ACK differs")
...
def _validate_challenge(challenge: str) -> bytes:
    if (
        ...
        raise ValueError("shared-direct V3 handshake challenge differs")
...
def _public_probe_child(challenge: str) -> int:
    ...
    _child_emit(
        ...
        {
            "schema_version": "legal-river-shared-direct-launcher-child-v3",
            "challenge_sha256": sha256(raw).hexdigest(),
            "literal_module": LITERAL_WORKER_MODULE,
            "spec_name": __spec__.name if __spec__ is not None else None,
            "runtime_name": __name__,
...
def _campaign_child() -> int:
    from .legal_river_quotient_cuda_shared_direct_sample_plan import (
...
def _decode_child_event(line: bytes) -> tuple[str, Mapping[str, object]]:
    if (
        ...
        raise ValueError("shared-direct V3 child line framing differs")
    ...
    if (
        ...
        raise ValueError("shared-direct V3 child event semantics differ")
...
def _run_child_process(
    ...
    *,
    ...
    if mode not in {_HANDSHAKE_MODE, _PUBLIC_CHILD_MODE, _CAMPAIGN_MODE}:
        raise ValueError("shared-direct V3 child mode differs")
    ...
    try:
        while not saw_eof:
            ...
            if remaining <= 0:
                raise TimeoutError("shared-direct V3 child wall crossed")
            try:
            ...
            if line is None:
                ...
                continue
            if len(line) > MAXIMUM_CHILD_LINE_BYTES:
                raise ValueError("shared-direct V3 child line exceeds cap")
            ...
            if count > MAXIMUM_EVENT_COUNT:
                raise ValueError("shared-direct V3 child event count exceeds cap")
        remaining = max(
            0.001,
    ...
    if len(stderr) > MAXIMUM_STDERR_BYTES:
        raise ValueError("shared-direct V3 child stderr exceeds cap")
...
def run_no_cuda_bootstrap_handshake() -> Mapping[str, object]:
    ...
    if outcome.return_code != 0 or outcome.stderr or len(events) != 1:
        raise RuntimeError("shared-direct V3 bootstrap child rejected")
    kind, event = events[0]
    if (
        ...
        raise ValueError("shared-direct V3 bootstrap semantics differ")
    return event
...
def public_launcher_probe() -> Mapping[str, object]:
    ...
    if outcome.return_code != 0 or outcome.stderr or len(events) != 1:
        raise RuntimeError("shared-direct V3 public child rejected")
    kind, child = events[0]
    parent = {
        "literal_module": LITERAL_WORKER_MODULE,
        "spec_name": __spec__.name if __spec__ is not None else None,
    ...
    if kind != "launcher_probe_child" or child != expected_child:
        raise ValueError("shared-direct V3 public child identity differs")
    if (
        _OUTPUT.exists()
        or _V1_OUTPUT.exists()
        or _RESERVED.exists()
        ...
        raise ValueError("shared-direct V3 public probe observed a result")
    return {
        "schema_version": "legal-river-shared-direct-public-launcher-probe-v3",
        "parent": parent,
        "child": dict(child),
    ...
    def append(
        ...
        *,
        ...
        if prospective > MAXIMUM_ARTIFACT_BYTES:
            raise RuntimeError("shared-direct V3 journal cap would be crossed")
...
def execute_owner_to_path(path: Path = _OUTPUT) -> Mapping[str, object]:
    if not isinstance(path, Path) or path != _OUTPUT:
        raise ValueError("shared-direct V3 owner output path differs")
    ...
    if (
        ...
        raise FileExistsError("shared-direct V3 protected authority exists")
    if path.exists():
        raise FileExistsError("shared-direct V3 result already exists")
    strict_git_metadata(result_created=False)
    handshake_passed = False
    terminal_name = "infrastructure_failure"
    terminal_reason = "V3 owner did not reach campaign"
    event_count = 0
    ...
    with DurableEvidenceJournalWriter.create(
        ...
        try:
            ...
            def append_event(
                ...
                if kind == "terminal_evidence":
                    if terminal_evidence is not None:
                        raise ValueError(
                            "shared-direct V3 terminal evidence repeated"
                        )
                    terminal_evidence = event

            remaining = PARENT_HARD_WALL_NS - (perf_counter_ns() - started)
            if remaining <= 0:
                raise TimeoutError("shared-direct V3 owner wall crossed")
            outcome = _run_child_process(
                _CAMPAIGN_MODE,
                challenge=None,
                wall_ns=remaining,
            ...
            if outcome.return_code != 0 or outcome.stderr:
                raise RuntimeError(
                    "shared-direct V3 campaign child failed: "
                    + outcome.stderr.decode("utf-8", errors="replace")[:1024]
                )
            if terminal_evidence is None:
                raise RuntimeError(
                    "shared-direct V3 campaign omitted terminal evidence"
                )
    ...
    if path.stat().st_size > MAXIMUM_ARTIFACT_BYTES:
        raise RuntimeError("shared-direct V3 retained journal exceeds cap")
    return outer
...
def main() -> None:
    ...
    if mode is not None:
        ...
        raise SystemExit("unknown shared-direct V3 child mode")
    if os.environ.get(_PUBLIC_PROBE_ENV) == "1":
        if len(sys.argv) != 1:
            raise SystemExit("shared-direct V3 probe accepts no arguments")
        print(
        ...
        return
    if len(sys.argv) != 1:
        raise SystemExit("shared-direct V3 owner accepts no arguments")
