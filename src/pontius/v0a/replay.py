...
def chip_depth_settlement(
    *,
    ...
    return tuple(payouts), tuple(pots)
...
class ReplayHost:
    ...
    def run(self, *, destination=None, run_root=None) -> ReplayOutcome:
        ...
        primary: FailureCode | None = None
        secondary: list[FailureCode] = []
        ...
        if primary is None and needs_showdown:
            feed(
                ShowdownResultEvent(
                    hand_id=fixture.hand_id,
                    event_index=len(accepted_events),
        ...
        if primary is None and runtime.betting_terminal:
            with runtime.bookkeeping():
                ...
                oracle_payouts, oracle_pots = self._oracle(
            ...
            if oracle_payouts != settlement.payouts:
                primary = FailureCode.SETTLEMENT_MISMATCH
            elif tuple(pot.amount for pot in settlement.pots) != tuple(
                amount for amount, _ in oracle_pots
            ):
                primary = FailureCode.SETTLEMENT_MISMATCH
        ...
        content = b""
        with runtime.publication_interval(publication):
            content = builder.close(
                ...
                decision_count=len(
                    [record for record in decisions if record.failure_reason is None]
                ),
            ...
            if destination is not None and run_root is not None:
                try:
                    ...
                    secondary.append(FailureCode.TRACE_WRITE_FAILED)
                    trace_digest = None
            else:
        ...
        receipt = HostCompletionReceipt(
            ...
            accounting_complete=totals.complete,
            passed=passed and trace_digest is not None,
            failure_reason=primary,
