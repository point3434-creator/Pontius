...
from .runtime import HandRuntime
from .trace import (
    TraceBuilder,
    TraceWriteError,
    semantic_sha256,
...
class ReplayHost:
    ...
    def run(self, *, destination=None, run_root=None) -> ReplayOutcome:
        ...
        causes: list[FailureCode] = []
        drained = 0

        def drain() -> None:
            """Move newly retained runtime closure causes into occurrence order."""

            nonlocal drained
            retained = runtime.closure_failures
            causes.extend(retained[drained:])
            drained = len(retained)

        def note(code: FailureCode) -> None:
            """Append a host-detected cause after flushing anything earlier."""

            drain()
            causes.append(code)

        def split() -> tuple[FailureCode | None, list[FailureCode]]:
            """First outcome cause is primary; everything else keeps its order.

            A post-delivery trace-write failure is a reporting failure by
            definition — ADR-0485 keeps such a failure in the ordered later
            collection rather than letting it replace the hand's first cause —
            so it is never promoted to primary even when it is the only cause.
            """

            outcome_causes = [c for c in causes if c is not FailureCode.TRACE_WRITE_FAILED]
            first = outcome_causes[0] if outcome_causes else None
            rest = [c for c in causes if c is not first]
            if first is not None:
                # drop exactly one occurrence of the promoted cause
                rest = list(causes)
                rest.remove(first)
            return first, rest
        ...
        if not causes and needs_showdown:
            feed(
                ShowdownResultEvent(
                    hand_id=fixture.hand_id,
                    event_index=len(accepted_events),
        ...
        if not causes and runtime.betting_terminal:
            with runtime.bookkeeping():
        ...
        try:
            publication_context = runtime.publication_interval(publication)
        except RuntimeError:
        ...
        drain()  # nothing is noted after finalization, so flush explicitly
