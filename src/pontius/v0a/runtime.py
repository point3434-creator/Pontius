...
class HandRuntime:
    ...
    def bookkeeping(self):
        ...
        outer = self._outer
        if outer is None:
            raise RuntimeError("bookkeeping requires a started hand")
        if self._finalized:
            raise RuntimeError("a finalized ledger measures no further work")
    ...
    def publication_interval(self, sink: list[float]):
        ...
        outer = self._outer
        if outer is None:
            raise RuntimeError("publication measurement requires a started hand")
        if self._finalized:
            raise RuntimeError("a finalized ledger measures no further work")
        outer.start_preparation_work()
        try:
    ...
    def finalize_accounting(self) -> None:
        ...
        try:
            outer.finalize()
        except (ClockInvalidError, ClockReversedError, RuntimeError):
            self._accounting_complete = False
    ...
    def _decide_inner(
        ...
        if work_cutoff_crossed:
            self._known_cutoff = True
        if ready.deadline_crossed:
