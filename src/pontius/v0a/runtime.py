...
class HandRuntime:
    ...
    def _record_closure_failure(self, error: BaseException) -> None:
        """Retain a closure fault's typed cause; never re-read a dead witness.
        ...
        typed code the frozen vocabulary does not have. In practice it follows
        an earlier clock fault whose code is already retained.
        """

        self._clock_dead = True
        self._accounting_complete = False
        if isinstance(error, ClockReversedError):
            self._closure_failures.append(FailureCode.CLOCK_REVERSED)
        elif isinstance(error, ClockInvalidError):
            self._closure_failures.append(FailureCode.CLOCK_INVALID)
    ...
    def dispatch(self, event: Event) -> DispatchOutcome:
        ...
        if self._outer is None:
            ...
            try:
                ...
                return self._clock_reject(error, event, wall_start_ns=None)
    ...
    def _release_boundary(self, boundary: object, terminal_at_entry: bool) -> None:
        ...
        except (ClockInvalidError, ClockReversedError, ValueError, RuntimeError):
            self._clock_dead = True
            self._accounting_complete = False
            return
    ...
    def _clock_reject(
        ...
        *,
        ...
        return self._from_failure(self._clock_hand_failure(error, wall_start_ns), event)
