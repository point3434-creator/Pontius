...
LAUNCHER_SPEC = importlib.util.spec_from_file_location(
    'workload_launcher',
    Path(__file__).resolve().parents[1] / 'tools/v0a_blueprint_workload.py')
launcher = importlib.util.module_from_spec(LAUNCHER_SPEC)
LAUNCHER_SPEC.loader.exec_module(launcher)


def run_no_launch_control(root, context):
    """Literal qualification prerequisite; real controller retention, no launch."""
    # This does not certify authority, qualification, population or source admission.
    captured = dict(context['source_files'])
    captured.update({path: (ROOT / path).read_bytes() for path, _ in launcher.TOOLS.values()})
    source = types.SimpleNamespace(root=ROOT, commit=context['source_commit'],
        tree=context['source_tree'], captured=captured,
        manifest=launcher.FrozenSource.raw_manifest(context['source_files']), check=lambda: None)
    loader, admission, native_subprocess = (launcher.load_tool, launcher.admit_run_runtimes,
                                           launcher.subprocess)
    fired, created = [], []

    def prerequisite_module(name, *args, **kwargs):
        module = loader(name, *args, **kwargs)
        if name == 'population':
            # Preserve real module imports; admitted-plan equality alone is a
            # literal prerequisite to the controller/finalizer-to-reader contract.
            module.freeze_plan = lambda *unused: context['plan']
        return module

    def interrupted_admission(*args):
        fired.append('runtime-admission')
        raise KeyboardInterrupt('finite prelaunch retention control')

    def observed_creation(*args, **kwargs):
        process = native_subprocess.Popen(*args, **kwargs)
        created.append(process)
        return process

    launcher.load_tool = prerequisite_module
    launcher.admit_run_runtimes = interrupted_admission
    launcher.subprocess = types.SimpleNamespace(**dict(vars(native_subprocess),
                                                      Popen=observed_creation))
    try:
        code = launcher.run(types.SimpleNamespace(run_root=root), source, report, [source], {})
        assert fired == ['runtime-admission'], fired
        assert not created, 'prelaunch interruption unexpectedly created a process'
        assert code != 0
        envelope = json.loads((root / 'run-envelope.json').read_bytes())
        assert envelope == dict(started_ns=None, finished_ns=None, stop='interrupted'), envelope
        return dict(code=code, envelope=envelope, trigger=fired[0], creations=0)
    finally:
        launcher.load_tool, launcher.admit_run_runtimes = loader, admission
        launcher.subprocess = native_subprocess
...
class ReportControls(unittest.TestCase):
    ...
    def test_census_preserves_missing_failed_and_unattempted(self):
        # Keep actual producer output in snapshot TEMP. Literal qualification
        # prerequisites do not establish qualification or invocation authority.
        root = Path(tempfile.mkdtemp(prefix='report-no-launch-'))
        context = full_run(root, inputs_only=True)
        control = run_no_launch_control(root, context)
        self.assertEqual(control['trigger'], 'runtime-admission')
        self.assertEqual(control['creations'], 0)
        read = lambda name: (root / name).read_bytes()
        refs = {ref['path']: ref for ref in report.parse_json(read('manifest.json'))['files']}
        # Isolate the frozen null defect before the separately covered full-source
        # reader: GREEN must also pass the unchanged public read_run below.
        retention, envelope = report.admit_retention(read, report.validate_plan(context['plan']),
                                                     refs)
        self.assertEqual(envelope, dict(started_ns=None, finished_ns=None, stop='interrupted'))
        self.assertTrue(retention['complete'])
        before = {name: read(name) for name in refs}
        loaded = report.read_run(root)
        self.assertEqual(loaded['summary']['planned'],
                         [cell['id'] for cell in context['plan']['cells']])
        self.assertEqual(loaded['summary']['census']['interrupted'],
                         [context['plan']['cells'][0]['id']])
        self.assertEqual(loaded['summary']['census']['unattempted'],
                         [cell['id'] for cell in context['plan']['cells'][1:]])
        self.assertFalse(loaded['summary']['clean'])
        self.assertEqual(loaded['run_envelope'], envelope)
        self.assertEqual(before, {name: read(name) for name in refs})
        plan = control_plan()
        ...
        self.failed_capture_control()

    def failed_capture_control(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            full_run(root, 'failed_safety')
            # This synthetic shell has an unattempted prefix before a later
            # failed session: the whole reader must reject that impossible run.
            with self.assertRaises(ValueError):
                report.read_run(root)
            read = lambda name: (root / name).read_bytes()
            refs = {ref['path']: ref for ref in report.parse_json(read('manifest.json'))['files']}
            plan, population = (report.parse_json(read(name)) for name in
                                ('plan.json', 'population.json'))
            qualified, _, claims = report.admit_qualification(plan, population, read, refs)
            selected, trajectories, _, _ = report.admit_queries(population, read)
            cell = next(cell for cell in plan['cells'] if cell['kind'] == 'session' and
                        report.parse_json(read('cells/' + cell['id'] + '/result.json'))['status']
                        == 'failed')
            record = report.parse_json(read('cells/' + cell['id'] + '/result.json'))
            runtime = next(row for row in plan['runtimes'] if row['id'] == cell['runtime'])
            report.admit_raw_result(record, cell, read, refs, selected, runtime, claims['run'],
                                    refs['plan.json'])
            reference = report.retained_session_reference(cell, runtime, qualified,
                {row['size']: row for row in population['artifacts']}, trajectories, read)
            facts = report.failed_session_facts(read(record['captures']['stdout']['path']),
                cell, reference, truncated=record['captures']['truncated'])
            retained = report.summarize([dict(record, failed_session_facts=facts)], plan)[
                'runtimes']['3.11']
    ...
    def test_census_rejects_duplicates_unknown_cells_and_wrong_types(self):
        # Literal native metadata proves reader arithmetic only; real held
        # process/creation controls live in the session suite.
        for defect in ('mixed', 'first', 'final', 'numeric-no-launch', 'null-with-launch',
                       'hidden-launch', 'orphan-launch', 'overlap', 'post-deadline',
                       'missing-budget',
                       'after-stop', 'result-stop', 'prelaunch-failed', 'prelaunch-interrupted',
                       'prelaunch-unattempted', 'pidless-success', 'pidless-sample',
                       'pidless-grant',
                       'no-native-observations', 'no-native-observations-file',
                       'no-native-profile', 'no-native-grant-file', 'pidless-observations',
                       'pidless-observations-file', 'pidless-profile', 'pidless-grant-file',
                       'no-native-exit', 'no-native-truncated', 'no-native-foreign-capture'):
            read, cells, refs = retention_clock_control(defect)
            with self.subTest(envelope=defect), self.assertRaises(ValueError):
                report.admit_retention(read, cells, refs)
        for state in ('ordinary', 'overshoot', 'pidless-attempt', 'no-native-intent',
                      'pidless-intent', 'no-native-empty-capture'):
            read, cells, refs = retention_clock_control(state)
            with self.subTest(envelope=state):
                _, envelope = report.admit_retention(read, cells, refs)
                self.assertEqual(envelope, report.parse_json(read('run-envelope.json')))
    ...
    def test_manifest_identity_census_and_read_is_nonmutating(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            full_run(root)
            source_rows = (root / 'admission/3.11-source.manifest').read_bytes().splitlines()
            self.assertEqual(source_rows, sorted(source_rows))
            self.assertNotEqual(source_rows, sorted(source_rows, key=lambda row: row[66:]))
    ...
    def test_manifest_rejects_escape_alias_missing_and_terminal_mismatch(self):
        for defect in ('source_path_order', 'source_duplicate', 'source_hash',
                       'short_runtime', 'short_cell', 'argv', 'environment', 'qualification',
    ...
    def test_reuse_exact_threshold_and_all_four_positive(self):
        ...
        # Literal group savings are 4 ns/hand; the independent component model
        # with P=20, R=24, C=2 has -6, 6, 15, 17.25 ns/hand at the four H values.
        model_plan, model_records = reuse_model_control()
        reduced = report.summarize(model_records, model_plan)['runtimes']
        for runtime, hands in (('3.11', [1, 2, 8, 32]), ('3.14', [1, 8])):
            with self.subTest(runtime=runtime):
                self.assertIn('reuse_groups', reduced[runtime])
                groups = reduced[runtime]['reuse_groups']
                self.assertEqual([group['hands'] for group in groups], hands)
                for group in groups:
                    self.assertEqual(group['planned_pairs'], 4)
                    self.assertEqual(group['completed_pairs'], 4)
                    self.assertEqual(group['incomplete_case_ids'], [])
                    self.assertEqual(group['observed']['savings_ns'], [4] * 4)
                    self.assertEqual(group['hash_per_hand_ns']['n'], 4 * group['hands'])
                    self.assertEqual(group['hash_per_hand_ns']['median'], 2)
                    for arm in ('fresh', 'retained'):
                        self.assertEqual(group['arms'][arm]['group_count'], 4)
                        self.assertEqual(group['arms'][arm]['query_count']['raw'],
                                         [group['hands']] * 4)
                        self.assertEqual(group['arms'][arm]['read_ns']['median'], 7)
                        self.assertEqual(group['arms'][arm]['decode_ns']['median'], 9)
                        self.assertEqual(group['arms'][arm]['memory_case_ids'],
                                         [pair[arm + '_case_id'] for pair in group['pairs']])
                    self.assertEqual(len(group['pairs']), 4)
                    self.assertEqual(group['pairs'][0]['retained']['hash_ns'],
                                     [2] * group['hands'])
                    self.assertEqual(group['pairs'][0]['fresh']['query_count'], group['hands'])
                    self.assertEqual(group['pairs'][0]['fresh']['read_ns'], 7)
                    self.assertEqual(group['pairs'][0]['fresh']['decode_ns'], 9)
                    self.assertEqual(group['pairs'][0]['retained']['memory_samples'],
                                     memory_samples())
                model = reduced[runtime]['reuse_model']
                self.assertEqual(model['status'], 'complete')
                self.assertIn('component', model['label'])
                self.assertEqual(model['coefficients_ns'],
                                 dict(fresh_prepare=20, retained_prepare=24, hash_per_hand=2))
                self.assertEqual(model['positive_component_crossing_hands'], 2)
                self.assertEqual([point['saving_per_hand_ns'] for point in model['points']],
                                 [-6, 6, 15, 17.25] if runtime == '3.11' else [-6, 15])
                self.assertFalse(reduced[runtime]['rules']['useful_preparation_reuse']['triggered'])
        missing = model_records[0]['cell_id']
        incomplete = report.summarize(model_records[1:], model_plan)['runtimes']['3.11']
        self.assertEqual(incomplete['reuse_groups'][0]['incomplete_case_ids'], [missing])
        self.assertEqual(incomplete['reuse_groups'][0]['completed_pairs'], 3)
        self.assertEqual(incomplete['reuse_model']['status'], 'unresolved')
        self.assertIsNone(incomplete['reuse_model']['coefficients_ns'])
        empty = report.summarize([], model_plan)['runtimes']
        self.assertEqual([row['hands'] for row in empty['3.11']['reuse_groups']], [1, 2, 8, 32])
        self.assertEqual([row['hands'] for row in empty['3.14']['reuse_groups']], [1, 8])
        for hash_cost in (20, 21):
            changed = copy.deepcopy(model_records)
            for record in changed:
                record['observations']['hash_ns'] = [hash_cost] * len(
                    record['observations']['hash_ns'])
            model = report.summarize(changed, model_plan)['runtimes']['3.11']['reuse_model']
            self.assertIsNone(model['positive_component_crossing_hands'])
        plan = control_plan()
...
def retention_clock_control(state):
    """Literal timing metadata; not native ownership or qualification evidence."""
    cells = {cell['id']: cell for cell in control_plan()['cells']}
    records = {ident: control_result(ident) for ident in cells}
    native = dict(cause=None, secondary=[], exit_code=0, truncated=False, samples=[],
        redirector_pid=7, launch_ns=100, outer_ns=200, cleanup_ns=5,
        cleanup=dict(verified=True, active=0), worker_failure=None, worker_grant=None)
    natives = {'c0': native}
    envelope = dict(started_ns=100, finished_ns=300, stop='budget_exhausted')
    budget = 3_600_000_000_000
    if state == 'mixed':
        envelope['started_ns'] = None
    elif state == 'first':
        envelope['started_ns'] = 99
    elif state == 'final':
        envelope['finished_ns'] = 301
    elif state == 'numeric-no-launch':
        natives.clear()
    elif state.startswith('no-native-'):
        natives.clear()
        envelope.update(started_ns=None, finished_ns=None, stop='budget_exhausted')
    elif state in ('null-with-launch', 'hidden-launch'):
        envelope.update(started_ns=None, finished_ns=None, stop='interrupted')
    elif state in ('overlap', 'post-deadline', 'after-stop', 'result-stop'):
        launch = {'overlap': 299, 'post-deadline': 100 + budget, 'after-stop': 310,
                  'result-stop': 310}[state]
        natives['c1'] = dict(native, redirector_pid=8, launch_ns=launch)
        envelope['finished_ns'] = launch + 200
        if state == 'after-stop':
            native.update(cause='worker_failed', exit_code=1)
            envelope['stop'] = 'worker_failed'
        elif state == 'result-stop':
            envelope['stop'] = 'parity_failed'
    elif state.startswith('prelaunch-'):
        natives = {'c1': native}
        envelope['stop'] = {'prelaunch-failed': 'worker_failed',
                            'prelaunch-interrupted': 'interrupted',
                            'prelaunch-unattempted': 'budget_exhausted'}[state]
    elif state in ('missing-budget', 'overshoot'):
        native['outer_ns'] = budget + 1
        envelope['finished_ns'] = 100 + budget + 1
        if state == 'overshoot':
            native.update(cause='worker_failed', secondary=['budget_exhausted'], exit_code=1)
            envelope['stop'] = 'worker_failed'
    elif state.startswith('pidless-'):
        native.update(redirector_pid=None, cause='budget_exhausted', exit_code=None)
        envelope.update(started_ns=None, finished_ns=None, stop='budget_exhausted')
        if state == 'pidless-success':
            native['exit_code'] = 0
        elif state == 'pidless-sample':
            native['samples'] = memory_samples()
        elif state == 'pidless-grant':
            native['worker_grant'] = {'worker_pid': 7}
    objects = {'retention.json': dict(version='workload-r002-retention-v1', complete=True,
                                     failures=[], missing=[]), 'run-envelope.json': envelope}
    for ident, value in natives.items():
        objects[f'cells/{ident}/supervision.json'] = value
        records[ident] = dict(records[ident], status='failed' if value['cause'] else 'completed',
            cause=value['cause'], secondary=value['secondary'], outer_ns=value['outer_ns'],
            exit_code=value['exit_code'], cleanup=value['cleanup'])
    if state == 'hidden-launch':
        records['c0'] = control_result('c0')
    elif state == 'result-stop':
        records['c0'].update(status='failed', cause='parity_failed')
    elif state.startswith('prelaunch-'):
        records['c0'] = dict(control_result('c0', state.removeprefix('prelaunch-')),
                             cause=envelope['stop'])
    if state.startswith(('no-native-', 'pidless-')):
        if state.endswith('-observations'):
            records['c0']['observations'] = {'worker_result': 1}
        for suffix, filename in (('observations-file', 'observations.json'),
                                 ('profile', 'profile.json'),
                                 ('grant-file', 'worker-grant.json'),
                                 ('intent', 'worker-grant-intent.json')):
            if state.endswith('-' + suffix):
                objects['cells/c0/' + filename] = {}
    if state.startswith('no-native-'):
        records['c0'].update(status='failed')
        if state == 'no-native-exit':
            records['c0']['exit_code'] = 0
        elif state == 'no-native-truncated':
            records['c0']['captures']['truncated'] = True
        elif state.endswith('-capture'):
            name, raw = ('cells/c0/stdout.bin', b'') if state == 'no-native-empty-capture' else (
                'admission/control-stdout.bin', b'foreign worker output\n')
            objects[name] = raw
            records['c0']['captures']['stdout'] = dict(path=name, bytes=len(raw),
                                                     sha256=hashlib.sha256(raw).hexdigest())
    objects.update({f'cells/{ident}/result.json': record for ident, record in records.items()})
    if state == 'orphan-launch':
        objects['cells/alien/supervision.json'] = dict(native, redirector_pid=8, launch_ns=310)
    encoded = {name: value if type(value) is bytes else launcher.encode(value)
               for name, value in objects.items()}
    refs = {name: dict(path=name, bytes=len(data), sha256=hashlib.sha256(data).hexdigest())
            for name, data in encoded.items()}
    return encoded.__getitem__, cells, refs


def reuse_model_control():
    """Literal arithmetic only: each group's observed saving differs from the model."""
    plan, records = control_plan(), []
    plan['cells'] = []
    plan['runtimes'].append(dict(plan['runtimes'][0], id='3.14', version='3.14.6'))
    for runtime, sizes in (('3.11', (1, 2, 8, 32)), ('3.14', (1, 8))):
        for hands in sizes:
            for repetition in range(4):
                for arm in ('fresh', 'retained'):
                    ident = f'v{runtime.replace(".", "")}-h{hands}-r{repetition}-{arm}'
                    plan['cells'].append(dict(id=ident, runtime=runtime, kind='reuse', size=8191,
                        parameters=dict(hands=hands, repetition=repetition, arm=arm,
                                        trajectory_ordinals=list(range(hands))),
                        argv=['D:/python.exe']))
                    records.append(completed_result(ident, dict(memory_samples=memory_samples(),
                        group_ns=100 * hands + 3 - (4 * hands if arm == 'retained' else 0),
                        prepare_ns=[20] * hands if arm == 'fresh' else [24],
                        hash_ns=[] if arm == 'fresh' else [2] * hands, read_ns=7, decode_ns=9,
                        query_count=hands, distinct_keys=hands,
                        context_ids=[f'context-{n}' for n in range(hands)])))
    return plan, records
...
def full_run(root, defect=None, *, inputs_only=False):
    """Synthetic full-denominator JSON schema fixture, never legal population evidence."""
    objects, native_intervals = {}, {}
    def raw(value):
        return (json.dumps(value, sort_keys=True, separators=(',', ':')) + '\n').encode()
    def put(name, value):
        if name.endswith('/supervision.json'):
            # Literal serial intervals; derive the envelope below from the
            # same retained records, not a numeric-only envelope assumption.
            prior = native_intervals.get(name)
            launch = prior['launch_ns'] if prior else (max(
                row['launch_ns'] + row['outer_ns'] for row in native_intervals.values()) + 10
                if native_intervals else 100)
            value = dict(value, launch_ns=launch)
            grant = None
    ...
    def claim(stage):
        value = dict(version='workload-r002-claim-v2', stage=stage, controller_pid=10,
            ...
            source_manifest_sha256=runtimes[0]['source_sha256'], authority_sha256=authority_sha256,
    ...
    # Literal source prerequisite, serialized by the real production producer.
    # These bytes do not claim qualification or native-admission evidence.
    protocol_path = 'docs/architecture/v0a-blueprint-workload-r001/execution-protocol.md'
    authority_path = 'docs/architecture/v0a-blueprint-workload-r001/invocation-authority.json'
    source_files = {protocol_path: b'literal workload protocol\n',
                    authority_path: b'{"control_prerequisite":true}\n',
                    'src/pontius/__init__.py': b'literal package\n',
                    'tools/v0a_event_adapter.py': b'literal event\n',
                    'tools/v0a_hand_adapter.py': b'literal hand\n',
                    'tools/v0a_rehearsal_driver.py': b'literal driver\n'}
    protocol_sha256 = hashlib.sha256(source_files[protocol_path]).hexdigest()
    authority_sha256 = hashlib.sha256(source_files[authority_path]).hexdigest()
    child_manifest_sha256 = hashlib.sha256(launcher.FrozenSource.raw_manifest(
        {name: data for name, data in source_files.items() if name not in
         (protocol_path, authority_path)})).hexdigest()
    source_manifest = launcher.FrozenSource.raw_manifest(source_files)
    if defect == 'source_path_order':
        source_manifest = b''.join(sorted(source_manifest.splitlines(keepends=True),
                                          key=lambda row: row[66:]))
    elif defect == 'source_duplicate':
        source_manifest += source_manifest.splitlines(keepends=True)[0]
    elif defect == 'source_hash':
        source_manifest = b'x' + source_manifest[1:]
    for runtime in runtimes:
        if inputs_only:
            runtime.update(source_root=str(ROOT), run_root=str(root))
    ...
    def environment(runtime):
        if inputs_only:
            return launcher.runtime_environment(Path(runtime['source_root']),
                root / 'temporary' / runtime['id'])
    ...
    if inputs_only:
        ...
        return dict(plan=plan, population=population, runtimes=runtimes, source_files=source_files,
                    source_commit='1' * 40, source_tree='2' * 40,
                    protocol_sha256=protocol_sha256, authority_sha256=authority_sha256)
    run_claim = claim('run')
    terminal = []
    for cell in plan['cells']:
        ...
        put(prefix + 'intent.json', dict(version='workload-r002-intent-v1', cell_id=cell['id'],
            cell=cell, source_sha256=runtime['source_sha256'], protocol_sha256=protocol_sha256,
    ...
    intervals = list(native_intervals.values())
    stopped_records = [json.loads(objects['cells/' + row['cell_id'] + '/result.json'])
                       for row in terminal if row['status'] != 'completed']
    stop = next((row['cause'] or row['secondary'][0] for row in stopped_records
                 if row['cause'] is not None or row['secondary']), None)
    put('run-envelope.json', dict(started_ns=intervals[0]['launch_ns'],
        finished_ns=intervals[-1]['launch_ns'] + intervals[-1]['outer_ns'], stop=stop))
