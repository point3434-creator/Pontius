...
class CampaignTests(unittest.TestCase):
    ...
    def test_comparisons_preserve_signs_controls_and_all_checkpoints(self):
        plan = self.runner.make_plan(Path('unused-holdout').resolve())
        records = []
        for step in (100, 1000, 10000):
            for index, case in enumerate(plan['cases']):
                values = dict(exact=0, uniform_equity_200=10, range_equity=12,
                              range_response=(9, 10, 11, 8)[index])
                for method, value in values.items():
                    records.append(dict(case=case['id'], method=method, iteration=step,
                                        full_game={'exploitability': value},
                                        full_game_fraction_of_pot={'exploitability': value / 10}))
        summary = self.runner.summarize(plan, records, [])
        self.assertEqual(summary['headline_iteration'], 10000)
        self.assertEqual(len(summary['comparisons']), 18)
        for step in (100, 1000, 10000):
            comparisons = [c for c in summary['comparisons'] if c['iteration'] == step]
            primary = next(c for c in comparisons if c['method'] == 'range_response'
                           and c['reference'] == 'uniform_equity_200')
            self.assertEqual([c['difference'] for c in primary['cases']], [-1, 0, 1, -2])
            self.assertEqual([c['case'] for c in primary['cases']],
                             [c['id'] for c in plan['cases']])
            self.assertEqual(primary['mean_difference'], -0.5)
            self.assertAlmostEqual(primary['mean_difference_fraction_of_pot'], -0.05)
            self.assertEqual(primary['sign_counts'], dict(lower=2, equal=1, higher=1))
            self.assertEqual(primary['case_pattern'], 'mixed')
            secondary = next(c for c in comparisons if c['method'] == 'range_response'
                             and c['reference'] == 'range_equity')
            self.assertEqual(secondary['mean_difference'], -2.5)
            self.assertEqual(secondary['case_pattern'], 'lower_or_equal')
        for record in records:
            record['full_game']['exploitability'] = 1
            record['full_game_fraction_of_pot']['exploitability'] = 0.1
        self.assertTrue(all(c['case_pattern'] == 'equal' and c['mean_difference'] == 0
                            for c in self.runner.summarize(plan, records, [])['comparisons']))
