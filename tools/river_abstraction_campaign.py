...
def summarize(plan, records, receipts):
    # Called only after the four complete grids have passed verify_case.
    steps = sorted({r['iteration'] for r in records})
    indexed = {(r['case'], r['iteration'], r['method']): r for r in records}
    means = {str(n): {m: fsum(indexed[c['id'], n, m]['full_game']['exploitability']
                              for c in plan['cases']) / 4 for m in METHODS} for n in steps}
    comparisons = []
    for n in steps:
        for reference in ('uniform_equity_200', 'range_equity'):
            for method in METHODS:
                if method == reference:
                    continue
                differences = []
                for case in plan['cases']:
                    candidate = indexed[case['id'], n, method]
                    control = indexed[case['id'], n, reference]
                    differences.append(dict(
                        case=case['id'], difference=candidate['full_game']['exploitability'] -
                        control['full_game']['exploitability'],
                        difference_fraction_of_pot=
                        candidate['full_game_fraction_of_pot']['exploitability'] -
                        control['full_game_fraction_of_pot']['exploitability']))
                values = [d['difference'] for d in differences]
                counts = dict(lower=sum(v < 0 for v in values), equal=sum(v == 0 for v in values),
                              higher=sum(v > 0 for v in values))
                pattern = ('mixed' if counts['lower'] and counts['higher'] else
                           'lower_or_equal' if counts['lower'] else
                           'higher_or_equal' if counts['higher'] else 'equal')
                comparisons.append(dict(
                    iteration=n, method=method, reference=reference, cases=differences,
                    mean_difference=fsum(values) / 4,
                    mean_difference_fraction_of_pot=
                    fsum(d['difference_fraction_of_pot'] for d in differences) / 4,
                    sign_counts=counts, case_pattern=pattern))
    return dict(complete=True, split=plan['split'], primary_criterion=CRITERION,
                headline_iteration=plan['iterations'], primary_reference='uniform_equity_200',
                secondary_reference='range_equity', difference_direction='method_minus_reference',
                mean_full_exploitability=means, comparisons=comparisons,
...
def execute_plan(path, expected_sha):
    ...
    try:
        ...
        summary = summarize(plan, records, receipts)
