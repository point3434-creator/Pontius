    ...
    try:
        ...
        for active in slots:
            ...
            supervise(cell['argv'], runtime_source.root, active['environment'],
                timeout_ns=remaining, worker=direct, gate=gate, outcome=active['native'])
            adopt_native(active)
...
def supervise(argv, root, environment, *, timeout_ns, stdout_limit=64 * 1024 * 1024,
    ...
    def grant_message(raw):
        ...
        for item in (*gate['checks'], *gate['owned']):
            item.check()
        grant_process.check()
        require(time.perf_counter_ns() - launched < timeout_ns, 'budget_exhausted')
    ...
    try:
        ...
        if process is not None:
            def reap():
                ...
                require(process_api().WaitForSingleObject(int(process._handle), 10000) == 0,
                        'cleanup_failed')
        ...
        for item in (sampler, grant_process):
            if item is not None:
                cleanup(lambda: require(item.api.WaitForSingleObject(item.handle, 10000) == 0,
                                        'cleanup_failed'))
