...
def reuse_report(rows, cells, n_fit):
    """Keep the planned H census and distinguish observations from a component model."""
    planned, observed, groups = {}, {cell['id']: record for cell, record in rows}, []
    for cell in cells.values():
        if cell['kind'] != 'reuse' or cell['size'] != n_fit:
            continue
        params = cell['parameters']
        arms = planned.setdefault(params['hands'], {}).setdefault(params['repetition'], {})
        require(params['arm'] not in arms, 'duplicate planned reuse arm')
        arms[params['arm']] = cell['id']
    for hands, repetitions in sorted(planned.items()):
        pairs, case_ids, incomplete = [], [], []
        for repetition, arms in sorted(repetitions.items()):
            case_ids.extend(arms.values())
            incomplete.extend(ident for ident in arms.values() if ident not in observed)
            if set(arms) != {'fresh', 'retained'} or not all(i in observed for i in arms.values()):
                continue
            fresh, retained = (observed[arms[arm]]['observations'] for arm in ('fresh', 'retained'))
            require(all(fresh[key] == retained[key] for key in
                        ('context_ids', 'query_count', 'distinct_keys')),
                    'paired reuse query/key census differs')
            pairs.append(dict(repetition=repetition, fresh_case_id=arms['fresh'],
                              retained_case_id=arms['retained'], fresh=fresh, retained=retained))
        reduced = reuse([pair['fresh']['group_ns'] for pair in pairs],
                        [pair['retained']['group_ns'] for pair in pairs], hands)
        arms = {arm: dict(group_count=len(pairs), **{
                    key: distribution([pair[arm][key] for pair in pairs])
                    for key in ('group_ns', 'query_count', 'distinct_keys',
                                'read_ns', 'decode_ns')},
                    memory_case_ids=[pair[arm + '_case_id'] for pair in pairs])
                for arm in ('fresh', 'retained')}
        groups.append(dict(hands=hands, planned_pairs=len(repetitions), completed_pairs=len(pairs),
            status='complete' if len(pairs) == len(repetitions) else 'unresolved',
            case_ids=case_ids, incomplete_case_ids=incomplete, pairs=pairs, observed=reduced,
            arms=arms, hash_per_hand_ns=distribution(
                [n for pair in pairs for n in pair['retained']['hash_ns']])))
    complete = bool(groups) and all(group['status'] == 'complete' for group in groups)
    model = dict(label='Preparation/hash component amortization model; not observed group wall '
                      'or a deployment break-even',
        assumptions=['Preparation and cached-byte hash costs are stationary for this source.',
                     'Matched query/lookup work cancels only in this component model.',
                     'Common read/decode and process launch are excluded from the difference.',
                     'Cached-byte hashing does not establish external source '
                     'or live-index freshness.'],
        formulas=dict(fresh='H*P', retained='R+H*C', saving_per_hand='P-C-R/H',
                      positive_component_crossing='floor(R/(P-C))+1 only when P>C'),
        status='complete' if complete else 'unresolved', coefficients_ns=None,
        coefficient_observations={}, case_ids=[ident for group in groups for ident in
                                               group['case_ids']],
        positive_component_crossing_hands=None, points=[])
    if complete:
        pairs = [pair for group in groups for pair in group['pairs']]
        samples = {
            'fresh_prepare': [n for pair in pairs for n in pair['fresh']['prepare_ns']],
            'retained_prepare': [n for pair in pairs for n in pair['retained']['prepare_ns']],
            'hash_per_hand': [n for pair in pairs for n in pair['retained']['hash_ns']]}
        model['coefficient_observations'] = {key: distribution(values)
                                             for key, values in samples.items()}
        coefficients = {key: result['median'] for key, result in
                        model['coefficient_observations'].items()}
        model['coefficients_ns'] = coefficients
        prepare, initial, hashed = (coefficients[key] for key in
                                    ('fresh_prepare', 'retained_prepare', 'hash_per_hand'))
        if prepare > hashed:
            model['positive_component_crossing_hands'] = math.floor(
                initial / (prepare - hashed)) + 1
        model['points'] = [dict(hands=group['hands'], fresh_component_ns=group['hands'] * prepare,
            retained_component_ns=initial + group['hands'] * hashed,
            saving_per_hand_ns=prepare - hashed - initial / group['hands']) for group in groups]
    return groups, model
...
def runtime_summary(rows, cells, n_fit):
    ...
    reuse_groups, reuse_model = reuse_report(by_kind['reuse'], cells, n_fit)
    groups = {}
    ...
    return {'rules': rules, 'unresolved': [name for name, result in rules.items()
            ...
            'profile_cases': profiles, 'reuse_groups': reuse_groups, 'reuse_model': reuse_model}
...
def summarize(records, plan, retention=None, envelope=None):
    ...
    runtimes = {runtime['id']: runtime_summary(
        [r for r in records if cells[r['cell_id']]['runtime'] == runtime['id'] and
         r['status'] == 'completed'], {ident: cell for ident, cell in cells.items()
                                      if cell['runtime'] == runtime['id']}, n_fit)
        for runtime in plan['runtimes']}
...
def admit_qualification(plan, population, read, refs):
    ...
    for runtime in plan['runtimes']:
        ...
        require(lines == sorted(lines) and source_rows.get(
...
def admit_raw_result(record, cell, read, refs, selected, runtime, claim, plan_ref):
    ...
    if record['outer_ns'] is None:
        require(record['status'] != 'completed' and prefix + 'supervision.json' not in refs,
                'missing outer interval conceals supervision')
        return
...
def admit_retention(read, cells, refs):
    ...
    require((envelope['started_ns'] is None) == (envelope['finished_ns'] is None) and
            (envelope['stop'] is None or envelope['stop'] in CAUSES), 'invalid run envelope')
    require({name for name in refs if name.endswith('/supervision.json')} <=
            {'cells/' + ident + '/supervision.json' for ident in cells},
            'unplanned native supervision')
    intervals, stopped = [], False
    for ident in cells:
        prefix = 'cells/' + ident + '/'
        record = parse_json(read(prefix + 'result.json'))
        record_stopped = (record['status'] != 'completed' or record['cause'] is not None or
                          bool(record['secondary']))
        name = prefix + 'supervision.json'
        native = parse_json(read(name)) if name in refs else None
        if native is None or native['redirector_pid'] is None:
            require(record['observations'] == {} and record['exit_code'] is None and
                not record['captures']['truncated'] and not any(prefix + filename in refs
                for filename in ('observations.json', 'profile.json', 'worker-grant.json')),
                'no-creation result contains worker output')
            for stream in ('stdout', 'stderr'):
                capture = prefix + stream + '.bin'
                require(record['captures'][stream] is None or
                        record['captures'][stream] == refs.get(capture),
                        'no-creation capture reference differs')
                if capture in refs:
                    require(read(capture) == b'', 'no-creation result contains worker capture')
        if native is None:
            require(record['outer_ns'] is None and record['status'] != 'completed',
                    'result lacks native interval evidence')
            stopped = stopped or record_stopped
            continue
        launch, outer = integer(native['launch_ns']), integer(native['outer_ns'])
        require(record['outer_ns'] == outer and type(record['outer_ns']) is int,
                'result conceals native interval')
        causes = ([native['cause']] if native['cause'] is not None else []) + native['secondary']
        require(all(type(cause) is str and cause in CAUSES for cause in causes),
                'invalid native envelope cause')
        if native['redirector_pid'] is None:
            require(causes and native['exit_code'] is None and not native['samples'] and
                    native['worker_grant'] is None and native['worker_failure'] is None and
                    record['status'] in ('failed', 'interrupted'),
                    'no-creation attempt contains worker execution')
        else:
            require(integer(native['redirector_pid']) > 0 and not stopped,
                    'native launch follows a stop')
            finish = integer(launch + outer)
            if intervals:
                require(launch >= intervals[-1][1] and
                        launch < intervals[0][0] + 3_600_000_000_000,
                        'native intervals overlap or launch follows deadline')
            intervals.append((launch, finish, causes))
        stopped = stopped or bool(causes) or record_stopped
    if not intervals:
        require(envelope['started_ns'] is None and envelope['finished_ns'] is None and
                envelope['stop'] is not None, 'no creation requires a stopped null envelope')
    else:
        require(integer(envelope['started_ns']) == intervals[0][0] and
                integer(envelope['finished_ns']) == intervals[-1][1],
                'run envelope differs from native intervals')
        if intervals[-1][1] > intervals[0][0] + 3_600_000_000_000:
            require('budget_exhausted' in intervals[-1][2] and envelope['stop'] is not None,
                    'deadline overshoot lacks retained budget cause')
    require(not stopped or envelope['stop'] is not None, 'native stop missing from run envelope')
